...
async function inspectMenu(page, menuName, failures) {
  ...
  for (const row of result.rows) {
    ...
    const dashboardReview = menuName === "dashboard" && (row.id === "help" || row.id === "open-source");
    const expectedBackground = dashboardReview ? result.warning : row.connected ? result.accentBright : result.muted;
...
export async function runSkinV1MenuContractAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  return { menus: ["dashboard", "settings", "cloud-story-mode", "local-story-mode"], status: "pass" };
}
