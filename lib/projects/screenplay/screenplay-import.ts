...
function key(value: string) {
  ...
  const normalizedSpacing = upperCaseValue.replace(/\s+/g, " ");
...
function characterName(value: string) {
  ...
  const withoutParenthetical = withoutMarker.replace(/\s*\([^)]*\)\s*\^?$/, "");
  const withoutDualDialogueMarker = withoutParenthetical.replace(/\^$/, "");
  return withoutDualDialogueMarker.trim();
}

function locationName(value: string) {
  const withoutPrefix = value.replace(
    /^\.?\s*(?:INT\.?\/?EXT\.?|EXT\.?\/?INT\.?|INT\.?|EXT\.?|I\/E\.?)\s*/i,
    "",
  );
  const withoutTime = withoutPrefix.replace(
    /\s+-\s+(?:DAY|NIGHT|MORNING|EVENING|LATER|CONTINUOUS|MOMENTS LATER|DAWN|DUSK|SAME)$/i,
    "",
  );
  const normalizedSpacing = withoutTime.replace(/\s+/g, " ");
  return normalizedSpacing.trim();
}
