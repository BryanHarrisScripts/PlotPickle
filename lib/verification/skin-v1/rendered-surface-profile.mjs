...
export async function collectRenderedSurfaceProfile(page, {
  ...
  return page.evaluate(({ surfaceName: name, rootSelector: selector, colorTokens: colors, contractTokens: tokens }) => {
    ...
    const typographyRole = (node) => {
      ...
      if (node.matches("[data-keycap],[data-dashboard-shortcut],[data-settings-shortcut],[data-library-shortcut]")) return { role: "keycap", source: "context" };
      ...
      if (
        node.matches("[role='menuitem'],[role='tab'],.pp-skin-v1-menu-item,[data-dashboard-menu-item],[data-settings-secondary-item],[data-library-nav]")
