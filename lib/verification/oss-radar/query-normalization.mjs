...
function queryReference(entry) {
  return {
    id: String(entry?.id || entry?.queryId || `${entry?.laneId || "unknown"}/${entry?.familyId || "default"}/${entry?.query || ""}`),
...
function uniqueQueryReferences(values) {
  const byId = new Map();
  for (const value of values || []) {
    const normalized = queryReference(value);
    if (normalized.id) byId.set(normalized.id, normalized);
  }
  return [...byId.values()].sort((left, right) => left.id.localeCompare(right.id));
}

export function normalizeRepository(raw, entry) {
  ...
  const queryRef = queryReference(entry);
  return {
    ...
    matchedLaneIds: uniqueSorted([queryRef.laneId]),
    matchedQueries: uniqueSorted([queryRef.query]),
    matchedQueryRefs: uniqueQueryReferences([queryRef]),
...
export function mergeDuplicateCandidates(candidates) {
  ...
  for (const candidate of candidates) {
    ...
    byRepository.set(key, {
      ...
      matchedQueryRefs: uniqueQueryReferences([...(current.matchedQueryRefs || []), ...(candidate.matchedQueryRefs || [])]),
