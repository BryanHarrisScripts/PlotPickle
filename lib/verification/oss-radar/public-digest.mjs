...
export function renderXReadyDigest({ reportDate, candidates = [] } = {}) {
  ...
  return [
    ...
    "",
    "Today's public sample takes one useful signal from each PlotPickle architecture area where available:",
    "",
    items,
    "",
    "The internal Radar reviews up to 21 repositories across seven architecture areas; this public digest is intentionally shorter.",
    "",
    "#OpenSource #AI #AgenticAI #StoryTechnology",
  ].join("\n");
}
...
export function renderRadarReviewEmail({ reportDate, candidates = [], publicDigest, reportUrl } = {}) {
  ...
  return {
    subject: `PlotPickle OSS Radar - ${date} - architecture review`,
    body,
  };
}
