...
test("#1977 Phase 2 selects five genuine findings and never pads", () => {
  ...
  assert.match(rendered.body, /Phase 2 uses \*\*WATCH\*\*/u);
...
test("#1977 Phase 2 allows unknown-license WATCH evidence without making it an adoption decision", () => {
...
test("#1977 Phase 2 state marker round-trips and same-day state is excluded from baseline history", () => {
  ...
  const parsed = parseDailyState(`text\n${marker}`);
  assert.equal(parsed.entries[0].repositoryStableId, "201");
  assert.equal(parseDailyState("unrelated comment"), null);
  assert.equal(parseDailyState("<!-- PLOTPICKLE-OSS-RADAR-STATE:not-json -->"), null);
...
test("#1977 Phase 2 suppresses unchanged recent reviews but permits material change", () => {
  ...
  const scoreChanged = { ...candidate, score: candidate.score + 5 };
  assert.equal(shouldResurface(scoreChanged, prior, "2026-09-13").reason, "score-change");

  const categoryChanged = { ...candidate, matchedLaneIds: ["writer-craft", "learn-education"] };
  assert.equal(shouldResurface(categoryChanged, prior, "2026-09-13").reason, "category-change");
});

test("#1977 Phase 2 activity changes resurface only after cooldown", () => {
  const candidate = fixture.candidates[0];
  const old = historyEntryForFinding({ ...candidate, primaryDisposition: "WATCH" }, "2026-08-01", null);
  const changed = { ...candidate, pushedAt: "2026-09-12T18:00:00Z" };
  assert.equal(shouldResurface(changed, old, "2026-09-13").reason, "activity-after-cooldown");

  const recent = { ...old, lastReviewedDate: "2026-09-05" };
  assert.equal(shouldResurface(changed, recent, "2026-09-13").allowed, false);
});
...
function githubIssueFixture({ issues = [], comments = {}, failSearch = false } = {}) {
  ...
  const fetchImpl = async (input, options = {}) => {
    ...
    if (commentsMatch && method === "GET") {
      ...
      return { ok: true, status: 200, json: async () => state.comments.get(number) || [] };
    }
...
test("#1977 Phase 2 reuses monthly issue and same-day rerun updates one comment", async () => {
  const api = githubIssueFixture({
    issues: [{ number: 900, title: "[OSS RADAR] September 2026", created_at: "2026-09-01T00:00:00Z" }],
    comments: { 900: [] },
  });
  const args = {
    ...
    auth: "fixture-auth",
    contract,
    discoveryResult: { candidates: fixture.candidates },
  ...
  assert.equal(first.monthlyIssueNumber, 900);
  assert.equal(api.state.issues.length, 1);
  assert.equal(api.state.comments.get(900).length, 1);
  assert.equal(api.state.comments.get(900)[0].id, first.commentId);
  assert.equal(parseDailyState(api.state.comments.get(900)[0].body).entries.length, 5);
});

test("#1977 Phase 2 month rollover creates one new issue and carries prior comments as history", async () => {
  const priorCandidate = fixture.candidates[0];
  const priorBody = dailyComment("2026-09-30", [priorCandidate]);
  const api = githubIssueFixture({
    issues: [{ number: 901, title: "[OSS RADAR] September 2026", created_at: "2026-09-01T00:00:00Z" }],
    comments: { 901: [{ id: 4999, body: priorBody }] },
  });
  const result = await publishDailyRadar({
    repository: "BryanHarrisScripts/PlotPickle",
    auth: "fixture-auth",
    contract,
    discoveryResult: { candidates: fixture.candidates },
...
test("#1977 Phase 2 issue API failure is explicit and writes stay inside issues/comments", async () => {
  ...
  await assert.rejects(
    ...
    /HTTP 500: fixture search failure/u,
  );

  const api = githubIssueFixture({ issues: [], comments: {} });
  await publishDailyRadar({
    repository: "BryanHarrisScripts/PlotPickle",
    auth: "fixture-auth",
    contract,
    discoveryResult: { candidates: fixture.candidates },
  ...
  assert.ok(api.state.calls.every((call) =>
    call.pathname === "/search/issues"
    || /^\/repos\/BryanHarrisScripts\/PlotPickle\/issues(?:\/\d+\/comments|\/comments\/\d+)?$/u.test(call.pathname)
  ));
});
