...
export async function runWebMcpSurfaceVisualAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    assert(settingsRows.map((item) => item.id).join(",") === "general,local-story-mode,cloud,node-info,agents", "WebMCP Settings discovery did not match the #2046 General + operational systems directory.", failures);
    assert(settingsRows.length === 5 && settingsRows.every((item) => item.connected), "WebMCP Settings discovery did not expose all five restored Settings destinations as connected.", failures);
  ...
  // Restored Local Story Mode, Cloud Story Mode, Node and Agents destinations are exercised by the Settings menu contract UAT.
