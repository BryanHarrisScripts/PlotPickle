...
function specificationsByStudy(candidatePackage, input) {
  const byStudy = new Map();
  const notApplicable = new Map();
  const derived = createCharacterDevelopmentStudySpecifications(candidatePackage);
  if (derived.status === "ready") {
    for (const item of derived.specifications) byStudy.set(item.studyId, item);
    for (const item of derived.notApplicable) notApplicable.set(item.studyId, item);
  }
  for (const item of Array.isArray(input?.studySpecifications) ? input.studySpecifications : []) {
    ...
    notApplicable.delete(studyId);
  }
  return { byStudy, notApplicable };
}
...
export function createCharacterDevelopmentGenerationPlan(candidatePackage, mediaStatus, input = {}) {
  ...
  const specificationSet = specificationsByStudy(candidatePackage, input);
  ...
  for (const study of candidates) {
    const notApplicableOutput = specificationSet.notApplicable.get(study.id);
    if (notApplicableOutput) {
      notApplicable.push(notApplicableOutput);
      continue;
    }
    const specification = specificationSet.byStudy.get(study.id);
    const prompt = text(specification?.prompt, 30_000);
    if (!prompt) return blocked("study-specification", `Study ${study.id} requires explicit canonical evidence or a revision-bound generation specification.`);
  ...
  return {
    ...
    plan: {
      ...
      studyIds: [...notApplicable.map((item) => item.studyId), ...jobs.map((job) => job.studyId)],
      notApplicable,
      jobs,
    },
  };
...
function publicPlan(plan) {
  return {
    ...
    notApplicableStudyIds: plan.notApplicable.map((item) => item.studyId),
  };
}
...
export async function runCharacterDevelopmentGeneration(server, candidatePackage, mediaStatus, input = {}, fetchImpl = fetch) {
  ...
  const generated = [];
  const failed = [];
  for (const item of planned.plan.notApplicable) {
    const recorded = recordCharacterDevelopmentStudyOutput(current, item.studyId, {
      ppfRevision: current.ppfBaseRevision,
      providerRoute: current.providerRoute,
      notApplicable: true,
      notApplicableReason: item.reason,
      notApplicableEvidenceRefs: item.evidenceRefs,
      createdAt: new Date().toISOString(),
    });
    if (recorded.status !== "ready") {
      failed.push({ studyId: item.studyId, studyType: "powers-effects", error: recorded.blocker?.detail || "The not-applicable study could not be recorded." });
    } else current = recorded.package;
  }
  ...
  return {
    status: failed.length ? (generated.length || planned.plan.notApplicable.length ? "partial" : "failed") : "ready",
    package: current,
