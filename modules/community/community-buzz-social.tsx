  ...
  return <>
    ...
    <aside className={styles.context} aria-label={`${target.label} details`}>
      ...
      <div className={styles.contextBody}>
        ...
        {target.kind === "dm" ? <section className={styles.contextCard}><span>Participants</span><h4>{participantNames.length || target.participants?.length || 0} participants</h4><p>{participantNames.join(" · ") || "BUZZ controls DM membership."}</p><small>PlotPickle does not describe this DM as end-to-end encrypted unless BUZZ provides that property.</small></section> : <section className={styles.contextCard}><span>Community callers</span><h4>{members.length} Great Hall directory entries</h4><p>These are Community callers, not a claim that every caller belongs to the selected private room.</p><div className={styles.memberList}>{members.slice(0, 8).map((member) => <div className={styles.memberRow} key={member.pubkey}><div><strong>{member.displayName}</strong><small>{member.presence || "offline"}</small></div><button type="button" disabled={!canPost} onClick={() => void onOpenDm(member.pubkey)}>Message</button></div>)}</div></section>}
