...
export function selectDailyFindings({ candidates = [], contract, history = new Map(), reportDate }) {
  ...
  const reviewQueue = [...fresh, ...repeated].slice(0, target);
  suppressed.history = Math.max(0, repeated.length - Math.max(0, reviewQueue.length - fresh.length));
  ...
  return { selected: qualified, belowThreshold, reviewQueue, suppressed, target, reviewTarget: target };
}
...
function findingMarkdown(candidate, index) {
  ...
  const status = candidate.reviewQualification === "qualified"
    ...
    : `Below qualification threshold by ${Number(candidate.qualificationGap || 0).toFixed(2)} points; included because it ranked in today's Top ${index <= 5 ? 5 : "review"}.`;
  return [
...
export function renderDailyReport({ reportDate, selection, contract, history = new Map() }) {
  ...
  const reviewNote = reviewQueue.length < reviewTarget
    ...
    : `The daily review queue contains ${reviewQueue.length}/${reviewTarget} real repositories. Qualification status is shown for context and never blanks the report.`;
