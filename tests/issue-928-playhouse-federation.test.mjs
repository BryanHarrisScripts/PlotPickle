...
test("presence can announce, withdraw and send a signed cross-Studio transport test",async()=>{const [source,gateway,page]=await Promise.all([read("build/playhouse-federation.ts"),read("build/playhouse-federation-gateway.ts"),read("app/community-presence/page.tsx")]);assert.match(source,/studio\.presence/);assert.match(source,/studio\.withdrawn/);assert.match(source,/studio\.test/);assert.match(gateway,/action === "withdraw"/);assert.match(gateway,/action === "test"/);assert.match(gateway,/\/api\/community-federation/);assert.match(page,/Community Presence/);assert.match(page,/Announce presence/);assert.match(page,/Withdraw presence/);assert.match(page,/Send signed transport test/);});

test("federation transport remains diagnostic evidence rather than human Great Hall chatter",async()=>{const projection=await read("lib/buzz/community-conversation.ts");assert.match(projection,/PLOTPICKLE_STUDIO_EVENT/);assert.match(projection,/studio\\\.\(\?:presence\|withdrawn\|test\)/);assert.match(projection,/return true/);});
