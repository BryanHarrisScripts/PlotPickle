...
2. Creates only missing rooms among all 13 configured PlotPickle Guildhall streams and forums, using each room's configured type, and keeps existing rooms.
...
5. Adds each discovered Agent to the public rooms contributed by the active PlotPickle Community plugin and to that Agent's functional Guildhall stream or forum, with the BUZZ `bot` role.
