...
export async function runWebMcpQaCli({
  ...
  if (selected.id !== "1") {
    ...
    if (census) {
      ...
      console.log(`[WEBMCP] Inventoried surfaces: ${summary.inventoriedSurfaces}`);
      console.log(`[WEBMCP] Standard governed surfaces: ${summary.standardWebmcpGovernedSurfaces}`);
      console.log(`[WEBMCP] Current Matrix navigation surfaces: ${summary.currentNavigationSurfaces}`);
      console.log(`[WEBMCP] Current Matrix governed surfaces: ${summary.activeGovernedSurfaces}`);
      console.log(`[WEBMCP] Active missing governance: ${summary.activeMissingGovernance.length}`);
      console.log(`[WEBMCP] In-transit inventory-only: ${summary.inTransitSurfaces.length}`);
      console.log(`[WEBMCP] Current-navigation failures: ${summary.currentNavigationFailures.length}`);
      console.log(`[WEBMCP] Inventory probe failures: ${summary.inventoryProbeFailures.length}`);
      console.log(`[WEBMCP] Skipped/state-only: ${summary.skippedUnsafe.length}`);
      console.log(`[WEBMCP] Inventory reconciliation: ${summary.inventoryReconciliationCoveragePct}%`);
      console.log(`[WEBMCP] Current-navigation reconciliation: ${summary.currentNavigationReconciliationCoveragePct}%`);
      console.log(`[WEBMCP] Governance coverage: ${summary.governanceCoveragePct}%`);
      for (const [label, values] of [
        ["Active missing governance", summary.activeMissingGovernance],
        ["Current-navigation failures", summary.currentNavigationFailures],
        ["Inventory probe failures", summary.inventoryProbeFailures],
