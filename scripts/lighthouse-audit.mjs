...
export function waitForWritableOpen(stream) {
  ...
  return new Promise((resolvePromise, reject) => {
    const cleanup = () => { stream.off("open", onOpen); stream.off("error", onError); };
...
export function closeWritable(stream) {
  ...
  return new Promise((resolvePromise, reject) => {
    const cleanup = () => { stream.off("close", onClose); stream.off("error", onError); };
    const onClose = () => { cleanup(); resolvePromise(); };
    const onError = (error) => { cleanup(); reject(error); };
    stream.once("close", onClose); stream.once("error", onError); stream.end();
  });
}
...
function startPreview(port, temporaryHome) {
...
function smokeResult(report, target, exitCode, consoleErrors) {
  ...
  if (!consoleClean) failures.push(`The route produced ${consoleErrors.length} serious browser console errors.`);
  if (browserErrorPage) failures.push("The route ended on a runtime, browser or cross-origin error page.");
  return { passed: failures.length === 0, failures, successfulDocument, documentTitle, metaDescription, consoleClean, browserErrorPage };
}
...
async function auditRoute({ baseUrl, route, mode, outputDirectory }) {
  ...
  const outputBase = join(outputDirectory, slug);
  const args = ["--yes", `lighthouse@${LIGHTHOUSE_VERSION}`, target, "--quiet", "--chrome-flags=--headless --no-sandbox --disable-gpu", "--output=json", "--output=html", `--output-path=${outputBase}`];
  ...
  for (const candidate of [`${outputBase}.report.json`, `${outputBase}.json`]) { try { await access(candidate); if (candidate !== jsonPath) await writeFile(jsonPath, await readFile(candidate)); break; } catch {} }
  for (const candidate of [`${outputBase}.report.html`, `${outputBase}.html`]) { try { await access(candidate); if (candidate !== htmlPath) await writeFile(htmlPath, await readFile(candidate)); break; } catch {} }
  try {
    const report = JSON.parse(await readFile(jsonPath, "utf8"));
    const categories = report.categories ?? {};
    const audits = Object.values(report.audits ?? {});
    const failedAudits = audits.filter((audit) => audit?.score !== null && audit?.score < 0.9 && audit?.scoreDisplayMode !== "notApplicable").map((audit) => ({ id: audit.id, title: audit.title, score: audit.score }));
    const accessibilityRefs = new Set((categories.accessibility?.auditRefs ?? []).map((item) => item.id));
    const seriousAccessibility = audits.filter((audit) => accessibilityRefs.has(audit?.id) && audit?.score === 0).map((audit) => ({ id: audit.id, title: audit.title }));
    const consoleErrors = report.audits?.["errors-in-console"]?.details?.items ?? [];
    return { route, status: exitCode === 0 ? "audited" : "audited-with-command-error", finalUrl: report.finalDisplayedUrl ?? report.finalUrl ?? target, smoke: smokeResult(report, target, exitCode, consoleErrors), scores: { performance: categories.performance?.score ?? null, accessibility: categories.accessibility?.score ?? null, bestPractices: categories["best-practices"]?.score ?? null, seo: categories.seo?.score ?? null }, failedAudits, seriousAccessibility, consoleErrors, files: { json: basename(jsonPath), html: basename(htmlPath), log: basename(logPath) } };
  } catch (error) {
    return { route, status: "failed", finalUrl: null, smoke: { passed: false, failures: [`No readable Lighthouse report was produced: ${error.message}`], successfulDocument: false, documentTitle: false, metaDescription: false, consoleClean: false, browserErrorPage: true }, scores: { performance: null, accessibility: null, bestPractices: null, seo: null }, failedAudits: [], seriousAccessibility: [], consoleErrors: [], files: { log: basename(logPath) } };
  }
}
...
async function checkRequiredAssets(baseUrl) {
  ...
  for (const assetPath of REQUIRED_ASSETS) {
    try { const response = await fetch(new URL(assetPath, baseUrl), { signal: AbortSignal.timeout(5_000) }); const body = await response.arrayBuffer(); results.push({ path: assetPath, status: response.status, bytes: body.byteLength, passed: response.ok && body.byteLength > 0 }); }
    catch (error) { results.push({ path: assetPath, status: null, bytes: 0, passed: false, error: error.message }); }
  }
  return results;
}

function summaryMarkdown(summary) {
  const lines = ["# PlotPickle Lighthouse diagnostic", "", `Generated: ${summary.generatedAt}`, `Mode: ${summary.mode}`, `Base URL: ${summary.baseUrl}`, `Smoke: ${summary.smoke.passed ? "PASS" : "FAIL"}`, "", "## Required metadata and brand assets", "", ...summary.requiredAssets.map((item) => `- ${item.passed ? "PASS" : "FAIL"} ${item.path}`), "", "Lighthouse category scores remain diagnostic evidence; category scores remain diagnostic and are not release thresholds.", "No story project was sent to a remote audit service."];
  return `${lines.join("\n")}\n`;
}
...
async function runMode(mode, reportDirectory, temporaryHome) {
  ...
  const preview = startPreview(port, temporaryHome);
  ...
  try {
    ...
    if (!summary.smoke.passed) console.warn(`Lighthouse diagnostic found ${routeFailures.length} routeFailures and ${assetFailures.length} assetFailures.`);
    return summary;
  } finally { await terminateProcessTree(preview, "PlotPickle local preview"); }
}
...
async function main() {
  ...
  try {
    ...
    else await run(process.platform === "win32" ? "npm.cmd" : "npm", ["run", "build"], { timeoutMs: 10 * 60_000 });
    const summaries = [];
    if (["smoke", "desktop", "mobile"].includes(command)) summaries.push(await runMode(command, reportDirectory, temporaryHome));
