...
async function recreateApprovedBranch(input: Record<string, unknown>) {
  ...
  if (syncValue.repository.toLowerCase() !== `${connection.owner}/${connection.repo}`.toLowerCase()) throw new Error("The verified synchronization state belongs to a different repository.");
