  ...
  try {
    for (const surfaceId of WEBMCP_STANDARD_SURFACE_TARGETS) {
      ...
      if (snapshot.controls > 0) {
        await page.evaluate(() => {
          document.querySelector("[data-webmcp-focus-sentinel='true']")?.remove();
          const active = document.activeElement;
          if (active instanceof HTMLElement && active !== document.body) active.blur();
          const sentinel = document.createElement("span");
          sentinel.setAttribute("data-webmcp-focus-sentinel", "true");
          sentinel.tabIndex = 0;
        ...
        await page.evaluate(() => document.querySelector("[data-webmcp-focus-sentinel='true']")?.remove());
