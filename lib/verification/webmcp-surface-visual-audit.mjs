...
export async function runWebMcpSurfaceVisualAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    assert(settingsRows.map((item) => item.id).join(",") === "general,story-mode,node-info,agents", "WebMCP Settings discovery did not match the #2106 Story Mode parent directory.", failures);
    assert(settingsRows.length === 4 && settingsRows.every((item) => item.connected), "WebMCP Settings discovery did not expose all four #2106 Settings destinations as connected.", failures);
