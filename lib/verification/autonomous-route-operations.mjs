...
export function parseAutonomousOperationProbeResult(value) {
  const source = String(value || "");
  const marker = "### Result";
  const markerIndex = source.indexOf(marker);
  const resultSection = markerIndex >= 0
    ? source.slice(markerIndex + marker.length).split(/\r?\n###\s/)[0]
    : source;
  const parsed = firstJsonObject(resultSection) || firstJsonObject(source);
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("Autonomous route operation probe returned no valid JSON object.");
  }
  return parsed;
}
...
export async function captureAutonomousRouteOperationProbe(session, route) {
  ...
  const result = (raw?.content || []).map((item) => item?.text || "").join("\n");
  return parseAutonomousOperationProbeResult(result);
}
