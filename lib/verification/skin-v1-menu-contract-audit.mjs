...
export async function runSkinV1MenuContractAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    if (settingsIds !== "general,local-story-mode,cloud,node-info,agents") {
      addFailure(failures, violation("SETTINGS", "settings-directory", "destinations", settingsIds, "general,local-story-mode,cloud,node-info,agents"));
    }

    const generalRow = page.locator("[data-settings-secondary-item='general']");
    await generalRow.focus();
    await page.keyboard.press("ArrowDown");
    await expectSelected(page, "[data-settings-secondary-item='local-story-mode']", "SETTINGS", "settings-local-story-mode", failures);
