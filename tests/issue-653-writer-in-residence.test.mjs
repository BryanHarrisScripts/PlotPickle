...
test("exploratory writer uses visible Playwright accessibility actions and never hidden browser state", async () => {
  ...
  assert.doesNotMatch(runner, /localStorage|document\.querySelector|window\./);
});

test("writer model receives only visible snapshot context and cannot edit code or GitHub", async () => {
  const runner = await read("scripts/run-writer-in-residence.mjs");
