...
export default function PlotPickleAgentsHost() {
  ...
  const refresh = useCallback(async (announce = false) => {
    try {
      ...
      setStatus(body);
      setNotice(announce ? "PlotPickle Agent compute refreshed." : "Local Story Mode and Cloud Story Mode supply these compute resources.");
    } catch (error) {
  ...
  return (
    <div style={shell} data-skin-v1-plotpickle-agents="true">
      <section style={panel} aria-labelledby="plotpickle-agents-title">
        <p style={{ margin: 0, color: "var(--pp-skin-accent-bright)", fontSize: 12, letterSpacing: ".08em" }}>SETTINGS / PLOTPICKLE AGENTS</p>
        <h1 id="plotpickle-agents-title" style={{ margin: "5px 0" }}>PLOTPICKLE AGENTS</h1>
        <p style={{ margin: 0, color: "var(--pp-skin-ink-soft)" }}>Assign compute already configured in Local Story Mode or Cloud Story Mode. Provider credentials and model setup stay in those Story Mode surfaces.</p>
      </section>
      ...
      <section style={panel} aria-labelledby="agent-default-title">
        ...
        <label style={{ display: "grid", gap: "var(--pp-skin-space-2)", maxWidth: 760 }}>
          <span>Use this when an Agent has no override</span>
          <select
            style={selectStyle}
          ...
          >
            <option value="active">Active Story Mode writing route{effectiveDefault !== "disabled" ? ` · ${effectiveDefault}` : " · currently Off"}</option>
      ...
      <section style={panel} aria-labelledby="agent-overrides-title">
        <h2 id="agent-overrides-title" style={{ marginTop: 0 }}>PER-AGENT OVERRIDES</h2>
        <div style={{ display: "grid", gap: "var(--pp-skin-space-3)" }}>
          {status.agents.map((agent) => {
            const override = status.overrides[agent.roleId];
            ...
            return (
              <article key={agent.roleId} style={{ padding: "var(--pp-skin-space-3)", border: "var(--pp-skin-border-thin) solid var(--pp-skin-line)", background: "var(--pp-skin-surface-0)", display: "grid", gridTemplateColumns: "minmax(220px, 1fr) minmax(280px, 1fr)", gap: "var(--pp-skin-space-3)", alignItems: "center" }}>
                <div>
                  <strong>{agent.displayName}</strong>
                  <p style={{ margin: "4px 0 0", color: "var(--pp-skin-ink-soft)" }}>{agent.title} · {agent.requestedCapabilityRole || "standard"} role</p>
                </div>
                <label style={{ display: "grid", gap: 4 }}>
                  <span style={{ fontSize: 12, color: "var(--pp-skin-ink-soft)" }}>{override ? "OVERRIDE" : "DEFAULT"}</span>
                  <select
                    style={selectStyle}
                    value={override || "default"}
                    disabled={Boolean(working)}
                    onChange={(event) => void save({ scope: "agent", agentId: agent.roleId, provider: event.target.value }, agent.roleId)}
                  >
                    <option value="default">Use PlotPickle default</option>
      ...
      <section style={panel}>
        ...
        <p style={{ marginBottom: 0, color: "var(--pp-skin-ink-soft)" }}>BUZZ identity, rooms, presence, keys and BUZZ runtime settings are not configured here. An Agent cannot grant itself a provider, expand its tools, alter canon authority or silently fall back to paid cloud compute.</p>
      </section>
