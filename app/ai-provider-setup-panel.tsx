...
async function jsonRequest<T>(path: string, method: "GET" | "POST" | "DELETE" = "GET", body?: object) {
...
export default function AiProviderSetupPanel({ provider }: { provider: ProviderId }) {
  ...
  async function saveAndTest() {
    ...
    try {
      const result = await jsonRequest<ConnectionResponse>(API, "POST", {
        provider,
        ...form,
        apiKey,
      });
  ...
  return (
    <section className={styles.panel} data-ai-provider-setup={provider} aria-labelledby={`${provider}-provider-setup-title`}>
      <header>
        <div>
          <p>Exact provider setup</p>
          <h2 id={`${provider}-provider-setup-title`}>{label}</h2>
