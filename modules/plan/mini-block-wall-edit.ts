...
import type { MiniBlock } from "../../lib/projects/structure";
  ...
  const blocks = project.blocks.map((block) => ({
    ...
    scenes: block.scenes.map((scene) => ({
      ...
      miniBlocks: scene.miniBlocks.map((mini) => {
        ...
        const next = { ...mini, ...patch, id: mini.id, number: mini.number };
        changed = Object.entries(patch).some(([key, value]) => mini[key as keyof MiniBlock] !== value);
        return changed ? next : mini;
      }),
    })),
  }));
