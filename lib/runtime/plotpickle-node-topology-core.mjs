...
export const PLOTPICKLE_PEER_RESOURCE_ROUTING_ENABLED = false;
...
export function normalizePlotPickleNodeEndpoint(value, trustScope) {
  ...
  if (!URL.canParse(endpointValue)) throw new Error("PlotPickle node endpoint must be a valid absolute URL.");
  const url = new URL(endpointValue);
  if (!["http:", "https:"].includes(url.protocol)) throw new Error("PlotPickle node endpoints must use HTTP or HTTPS.");
  if (url.username || url.password) throw new Error("PlotPickle node endpoint URLs cannot contain credentials.");
  if ((url.pathname && url.pathname !== "/") || url.search || url.hash) throw new Error("PlotPickle node endpoints must be an origin only, without a path, query, or fragment.");
  if (trustScope === "local" && !isLoopbackNodeHostname(url.hostname)) throw new Error("Local PlotPickle nodes must use a loopback endpoint.");
  if (trustScope === "lan" && !isPrivateLanNodeHostname(url.hostname)) throw new Error("LAN PlotPickle nodes must use a private/local-network hostname or address.");
  if (trustScope === "internet") {
    if (url.protocol !== "https:") throw new Error("Internet-facing PlotPickle nodes require HTTPS.");
    if (isLoopbackNodeHostname(url.hostname) || isPrivateLanNodeHostname(url.hostname)) throw new Error("Internet-facing PlotPickle nodes cannot use loopback or private-LAN endpoints.");
  }
  return url.origin;
}
...
export function normalizePlotPickleNodeDescriptor(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) throw new Error("PlotPickle node descriptor must be an object.");
  ...
  const hardware = input.hardware && typeof input.hardware === "object" && !Array.isArray(input.hardware)
    ? {
        platform: String(input.hardware.platform || "unknown"), architecture: String(input.hardware.architecture || "unknown"),
        cpuModel: String(input.hardware.cpuModel || ""), ramGb: Number(input.hardware.ramGb || 0),
        gpuName: String(input.hardware.gpuName || ""), gpuGeneration: String(input.hardware.gpuGeneration || ""),
        gpuMemoryGb: Number(input.hardware.gpuMemoryGb || 0),
        memoryModel: ["system", "discrete", "unified", "unknown"].includes(input.hardware.memoryModel) ? input.hardware.memoryModel : "unknown",
      }
    : null;
  return Object.freeze({ id, label, mode: input.mode, trustScope: input.trustScope, endpoint, enabled: input.enabled === true, readiness: input.readiness, capabilities: Object.freeze(capabilities), hardware: hardware ? Object.freeze(hardware) : null });
}
...
export function createLocalDesktopPlotPickleNode(input = {}) {
  ...
  return normalizePlotPickleNodeDescriptor({
    id: input.id || "local-desktop", label: input.label || "This PlotPickle", mode: "desktop", trustScope: "local",
    endpoint: input.endpoint || "http://127.0.0.1:4173", enabled: true, readiness: input.readiness || "ready",
    capabilities, hardware: input.hardware || null,
  });
}

function localModeRank(node) {
  return node.mode === "hybrid" ? 0 : node.mode === "compute" ? 5 : node.mode === "desktop" ? 10 : 15;
}
...
export function selectPlotPickleNode(nodes, request = {}) {
  ...
  const allowedTrustScopes = Array.isArray(request.allowedTrustScopes) && request.allowedTrustScopes.length ? request.allowedTrustScopes : ["local"];
  for (const scope of allowedTrustScopes) assertChoice(scope, TRUST_SCOPES, "Allowed PlotPickle node trust scope");
  if (!allowedTrustScopes.includes("local")) return null;
  const candidates = nodes
    .map(normalizePlotPickleNodeDescriptor)
    .filter((node) => node.trustScope === "local")
    ...
    .sort((left, right) => localModeRank(left) - localModeRank(right) || left.id.localeCompare(right.id));
