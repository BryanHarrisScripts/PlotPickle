...
export async function runWebMcpSurfaceVisualAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    assert(Array.isArray(settingsAvailable?.settings) && settingsAvailable.settings.some((item) => item.id === "local-story-mode" && item.connected), "WebMCP Settings discovery did not expose connected Local Story Mode.", failures);
    assert(Array.isArray(settingsAvailable?.settings) && settingsAvailable.settings.some((item) => item.id === "node-info" && item.connected), "WebMCP Settings discovery did not expose connected Node Info.", failures);
