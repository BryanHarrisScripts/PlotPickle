...
async function shellGeometry(rootLocator) {
  return rootLocator.evaluate((root) => {
    const card = root.querySelector(":scope > .pp-skin-v1-panel");
    const brand = card?.querySelector(":scope > .pp-skin-v1-title[data-skin-v1-standard-header='true']");
...
export async function captureWebMcpProfileGateState({
  ...
  try {
    ...
    const rootLocator = page.locator(expectedSelector(state));
    await rootLocator.waitFor({ state: "visible", timeout: 20_000 });
    await page.waitForFunction(
      () => document.documentElement.dataset.plotpickleSkin === "skin-v1"
        && Boolean(document.documentElement.dataset.plotpickleSkinTheme),
      undefined,
      { timeout: 20_000 },
    );
    ...
    const geometry = await shellGeometry(rootLocator);
    if (geometry.surface !== "LOGON" || !geometry.card || !geometry.canonicalHeader || geometry.headerCells.length !== 3) {
...
export function assertProfileGateContinuity(initializing, locked) {
  ...
  const aBrand = initializing?.geometry?.brand;
  const bBrand = locked?.geometry?.brand;
  if (!a || !b || !aBrand || !bBrand) {
    throw new Error("Skin V1 LOGON continuity requires both rendered panel and header geometries.");
  }
  const panelDeltas = {
    x: Math.abs(a.x - b.x),
    y: Math.abs(a.y - b.y),
  ...
  const headerDeltas = {
    x: Math.abs(aBrand.x - bBrand.x),
    y: Math.abs(aBrand.y - bBrand.y),
    width: Math.abs(aBrand.width - bBrand.width),
    height: Math.abs(aBrand.height - bBrand.height),
  };
  const initializingHeader = JSON.stringify(initializing.geometry.headerCells);
  const lockedHeader = JSON.stringify(locked.geometry.headerCells);
  if (initializingHeader !== lockedHeader) {
    throw new Error(`Skin V1 LOGON header changed between initialization and locked state (${initializingHeader} -> ${lockedHeader}).`);
  }
  if ([...Object.values(panelDeltas), ...Object.values(headerDeltas)].some((delta) => delta > 1)) {
    throw new Error(`Skin V1 LOGON shell moved between initialization and locked state (panel ${JSON.stringify(panelDeltas)}, header ${JSON.stringify(headerDeltas)}).`);
  }
  return true;
}
