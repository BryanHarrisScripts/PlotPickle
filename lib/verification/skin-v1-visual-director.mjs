...
async function navigateAndCollect(page, surface) {
  ...
  if (["community", "settings", "profile"].includes(surface)) {
    ...
    await visibleClick(page, "[data-dashboard-menu-item='profile']", "Profile Dashboard row");
    await page.locator(READINESS.profile).first().waitFor({ state: "visible", timeout: 15_000 });
    const button = page.locator("button:visible").filter({ hasText: surface === "local-ai" ? "LOCAL STORY MODE" : "NODE" }).first();
