...
import { runWebMcpSurfaceCensusProfile } from "../browser-probes/surface-census.mjs";
import {
    ...
    : profile.id === "3"
      ...
      : profile.id === "4"
        ...
        : profile.id === "5"
          ...
          : profile.id === "6"
            ? await runWebMcpSurfaceCensusProfile(args)
            : null;
...
export async function runWebMcpQaProfile({
  ...
  const runId = safeRunId(selected.id === "7" ? "webmcp-full-qa" : `webmcp-${selected.key}`);
  const startedAt = new Date().toISOString();
  const artifactRoot = path.resolve(
    repoRoot,
    selected.id === "7" ? WEBMCP_FULL_QA_ARTIFACT_ROOT : ".artifacts/webmcp-qa",
    runId,
  );
  ...
  for (const id of WEBMCP_FULL_QA_ORDER) {
    ...
    await onEvent?.({ type: "stage", label: `FULL QA ${profile.id}/6 · ${profile.label}`, detail: profile.description });
    try {
      const result = await executeProfile({
        profile,
        serverUrl,
  ...
  const overall = results.some((result) => result.status === "FAIL" || Number(result.blockers) > 0 || Number(result.advisories) > 0) ? "FAIL" : "PASS";
  ...
  return {
    ...
    latestZipPath: artifacts.latestZipPath,
