...
export function evaluateAutonomousRouteOperation(route, evidence = {}, probe = {}, context = {}) {
  ...
  if (route.id === "story-decisions") {
    if (probe?.decisionQueueReachable !== true) {
      return { ...receipt, error: "Story Decision gateway could not be reached through the autonomous Guest application session." };
    }
    const actionable = integer(probe?.actionableDecisionCount);
    if (actionable > 0) {
    ...
    return { ...receipt, succeeded: true, outcome: "completed-no-change", decisionCount: integer(probe?.decisionCount) };
  }
  ...
  if (["storyboard", "production-shots", "previs-animatic"].includes(route.id)) {
    ...
    if (integer(probe?.staleProductionTargets) > 0) {
    ...
    return {
      ...
      readinessItems: integer(probe?.readinessItems),
      storyDecisionTargets: integer(probe?.storyDecisionTargets),
    };
  }
...
export async function captureAutonomousRouteOperationProbe(session, route) {
  ...
  try { return JSON.parse(textResult.slice(start, end + 1)); } catch { return {}; }
}
