...
function ossRulesSection(value) {
  ...
  const summary = findings.map((item, index) => {
    const source = item.ossRulesUrl || item.sourceUrl;
    const origin = item.sourceKind === "pattern-library" ? "OSS Rules pattern library" : item.repository;
    return `${index + 1}. **${item.pattern}** — ${origin} (${item.area}): ${item.meaning} ${item.plotPickleFit} **${item.disposition}**. [OSS Rules source](${source})`;
  });
  return [
    ...
    ...(value?.status === "unavailable" ? ["OSS Rules enrichment unavailable; GitHub-first Radar results remain intact."] :
      summary.length ? summary : ["OSS Rules was available, but no bounded observation could be produced."]),
  ].join("\n");
}
