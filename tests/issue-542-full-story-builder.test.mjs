...
test("#542 launches the Full Story Builder beside the Windows server and saves through the local project archive", async () => {
  const [batch, vite, gateway, worker, folder, localGateway, packageJson] = await Promise.all([read("Start-PlotPickle.bat"), read("vite.config.ts"), read("build/full-story-builder-gateway.ts"), read("scripts/full-story-builder-agent.mjs"), read("lib/projects/persistence/project-folder.ts"), read("build/local-project-gateway.ts"), read("package.json")]);
