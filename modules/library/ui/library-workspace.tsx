...
import AverySessionHistory from "../../dashboard/ui/avery-session-history/index";
...
function openActiveProject() {
  // A full local reload guarantees every workspace reopens the newly active PPF snapshot.
...
function CatalogCard({ item, sourceKind, onLoad }: {
  ...
  return (
    <article className={styles.card} data-library-catalog-id={item.id}>
      <div className={styles.visual} data-visual-kind={item.id} aria-hidden="true">
      ...
      <div className={styles.cardBody}>
        ...
        <button className={styles.primaryButton} onClick={onLoad} type="button">
          {sourceKind === "example" ? "Load & Explore" : "Start from Preset"}
        </button>
      </div>
    </article>
  );
...
function StoryCard({ item, activeProjectId, onOpen }: {
  ...
  return (
    <article className={`${styles.card} ${active ? styles.activeCard : ""}`} data-library-story-id={item.id}>
      ...
      <div className={styles.cardBody}>
        ...
        <button className={active ? styles.secondaryButton : styles.primaryButton} onClick={active ? openActiveProject : onOpen} type="button">
          {active ? "Resume" : "Open Story"}
        </button>
      </div>
    </article>
  );
  ...
  function confirmLoad() {
    ...
    try {
      if (pending.kind === "story") {
        ...
        createLibraryWorkingCopy({
          sourceProject: pending.item.project,
  ...
  return (
    <main className={styles.workspace} aria-labelledby="library-title" data-library-workspace="v1">
      <header className={styles.hero}>
        <div>
          <p className={styles.eyebrow}>Local story archive</p>
          <h1 id="library-title">Library</h1>
          <p>Examples &amp; Stories</p>
        </div>
        <aside aria-label="Active story">
          <span>Active story</span>
          <strong>{activeProject?.title || "Opening your Library…"}</strong>
          <small>Your work stays local and is saved before every story switch.</small>
        </aside>
      </header>
      ...
      <nav aria-label="Library filters" className={styles.tabs}>
        {TABS.map((item) => (
          ...
          >
            {item.label}
            {item.id === "stories" ? <span>{stories.length}</span> : null}
          </button>
        ))}
      </nav>
      ...
      {tab === "stories" ? (
        <section aria-labelledby="my-stories-title" className={styles.section}>
          <div className={styles.sectionHeading}>
            ...
            <p>Open any saved Human project without mixing it with Avery’s read-only Writer-in-Residence sessions. Those sessions stay above the story shelf and never become the active Human project.</p>
          </div>
          {stories.length ? (
            ...
            <div className={styles.empty}>
              <h3>Your shelves are ready.</h3>
              <p>Explore a complete example or choose a genre preset to create your first local working story.</p>
              <button className={styles.primaryButton} onClick={() => setTab("featured")} type="button">Browse Featured Examples</button>
            </div>
          )}
        </section>
      ) : (
        <section aria-labelledby={`${tab}-title`} className={styles.section}>
          <div className={styles.sectionHeading}>
            <div>
              <p className={styles.eyebrow}>{tab === "featured" ? "Immutable source projects" : "Canonical PPF starters"}</p>
              <h2 id={`${tab}-title`}>{tab === "featured" ? "Featured Examples" : "Genre Presets"}</h2>
            </div>
            <p>{tab === "featured"
              ? "Load a user-owned working copy, then explore it through the normal PlotPickle workflow. The packaged source never changes."
              : "Each preset fills only fields supported by the current PPF model, leaving your creative decisions open."}</p>
          </div>
          <div className={styles.grid}>
            {visibleCatalog.map((item) => (
              <CatalogCard
                item={item}
                key={item.id}
                onLoad={() => setPending({ kind: "catalog", sourceKind: tab === "featured" ? "example" : "preset", item })}
                sourceKind={tab === "featured" ? "example" : "preset"}
              />
            ))}
          </div>
        </section>
      ...
      {pending ? (
        <div className={styles.dialogBackdrop} role="presentation">
          <section aria-labelledby="library-load-title" aria-modal="true" className={styles.dialog} role="dialog">
            <p className={styles.eyebrow}>Safe project switch</p>
            <h2 id="library-load-title">Load this project?</h2>
            <p>Your current work will be saved as a local story before PlotPickle switches projects.</p>
            <strong>{pending.item.title}</strong>
            <div>
              <button className={styles.secondaryButton} onClick={() => setPending(null)} type="button">Keep Current Story</button>
              <button className={styles.primaryButton} onClick={confirmLoad} type="button">Save &amp; Switch</button>
            </div>
          </section>
        </div>
      ) : null}
