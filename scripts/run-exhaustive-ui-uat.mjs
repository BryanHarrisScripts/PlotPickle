...
async function main() {
  ...
  await writeFile(path.join(artifactRoot, "exhaustive-ui-uat-report.md"), [
    ...
    `Genuine dead controls: ${audit.totals.dead}`,
    `Browser harness failures: ${audit.totals.harness}`,
    `Accessibility-target failures: ${audit.totals.accessibility}`,
    `Unreached controls: ${audit.totals.unreached}`,
    "",
    "## Screen coverage",
    ...(audit.screens || []).map((screen) => `- ${screen.label}: ${screen.complete ? "PASS" : "FAIL"} — ${screen.passed}/${screen.safe} safe controls; blocked=${screen.blocked}; dead=${screen.dead}; harness=${screen.harness}; accessibility=${screen.accessibility}; unreached=${screen.unreached}`),
    "",
    "## GitHub-reportable product findings",
    ...(findings.length ? findings.map((finding) => `- ${finding.fingerprint}: ${finding.message}`) : ["- No reportable exhaustive UAT product defects."]),
    "",
    "## Harness/accessibility observations",
  ...
  await bestEffortLiveBuzzActivity({
    ...
    summary: `Exhaustive code/UI/UX UAT ${report.overall}: ${audit.totals.passed}/${audit.totals.safe} safe controls passed; dead=${audit.totals.dead}; harness=${audit.totals.harness}; accessibility=${audit.totals.accessibility}; unreached=${audit.totals.unreached}; reportable=${findings.length}.`,
