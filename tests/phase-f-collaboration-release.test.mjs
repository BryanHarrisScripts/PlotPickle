...
test("PlotPickle 1.0 candidate defines a portable .ppf boundary", async () => {
  ...
  assert.match(packageJson.version, /^1\.0\.0-rc\.\d+(?:\.\d+)*$/);
  assert.match(packageJson.scripts["test:full"], /phase-f-collaboration-release\.test\.mjs/);
  for (const phrase of ["plotpickle-project-file", "PPF_FORMAT_VERSION", "createPortableProjectFile", "parsePortableProjectFile", "integrityValid", "portableProjectFileName", "PLOTPICKLE_VERSION"]) {
