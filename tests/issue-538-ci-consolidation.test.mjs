...
test("issue #538 keeps deep validation available outside the ordinary PR runner path", async () => {
  const [qa, story, windows, visual, ben] = await Promise.all([
  ...
  for (const workflow of [qa, story, windows, visual, ben]) assert.doesNotMatch(workflow, /^  pull_request:/m);
...
test("issue #538 historical gate responsibilities remain inside the consolidated pair", async () => {
  const [prGate, productGate] = await Promise.all([
    source(".github/workflows/pr-gate.yml"),
    source(".github/workflows/product-gate.yml"),
  ]);

  assert.match(prGate, /audit:credentials/);
  assert.match(prGate, /BEN deterministic code-quality delta/);
  assert.match(prGate, /Validate repository architecture/);
  assert.match(prGate, /Production web build/);
  assert.match(productGate, /Visual Readiness deterministic contracts/);
  assert.match(productGate, /issue-349-common-overlays-ux/);
