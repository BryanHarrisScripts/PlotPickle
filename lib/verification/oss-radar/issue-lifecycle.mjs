...
const DAILY_MARKER = /<!-- PLOTPICKLE-OSS-RADAR-DAY:(\d{4}-\d{2}-\d{2}) -->/u;
...
async function updateMonthlyIssueBody({ repository, issueNumber, body, auth, fetchImpl }) {
  const repo = repoPath(repository);
  return requestJson(`${API}/repos/${repo}/issues/${Number(issueNumber)}`, {
    auth,
    fetchImpl,
    method: "PATCH",
    body: { body },
...
function reportDateFromComment(comment) {
  return String(comment?.body || "").match(DAILY_MARKER)?.[1] || null;
}

function monthlyIssueBodyWithNewestFirstIndex({ value, comments, repository, issueNumber }) {
  const reports = (comments || [])
    .map((comment) => ({ comment, date: reportDateFromComment(comment) }))
    .filter((entry) => entry.date && entry.comment?.id)
    .sort((left, right) => right.date.localeCompare(left.date) || Number(right.comment.id) - Number(left.comment.id));
  const index = reports.length
    ? reports.map(({ comment, date }) => `- [${date}](https://github.com/${repoPath(repository)}/issues/${Number(issueNumber)}#issuecomment-${Number(comment.id)})`)
    : ["- No daily reports yet."];
  return [
    monthlyIssueBody(value),
    "",
...
export async function publishDailyRadar({
  ...
  const indexedComments = existing
    ? currentComments.map((item) => item.id === existing.id ? { ...item, body: rendered.body } : item)
    : [...currentComments, comment];
  await updateMonthlyIssueBody({
    repository,
    issueNumber: monthlyIssue.number,
    body: monthlyIssueBodyWithNewestFirstIndex({ value: now, comments: indexedComments, repository, issueNumber: monthlyIssue.number }),
    auth,
    fetchImpl,
  });
