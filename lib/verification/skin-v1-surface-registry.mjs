...
const grammarUrl = new URL("../../config/skin-v1-surface-grammar.json", import.meta.url);
...
function loadCanonicalRegistry() {
  ...
  const grammar = JSON.parse(readFileSync(grammarUrl, "utf8"));
  if (
    registry.formatProfileContract?.issue !== 2226
    || registry.formatProfileContract?.phase !== 2
    || registry.formatProfileContract?.requiredForCapturePolicy !== "standard"
    || grammar.profileContract?.issue !== 2226
    || grammar.profileContract?.phase !== 2
  ) {
    throw new Error("Skin V1 canonical Surface Registry must define the Issue #2226 Phase 2 format-profile contract.");
  }
  const profileCollections = {
    layout: grammar.layoutArchetypes,
    shell: grammar.shellProfiles,
    frame: grammar.frameProfiles,
    selectedState: grammar.selectedStateProfiles,
    typography: grammar.typographyProfiles,
  };
  for (const surface of standards) {
    const profile = surface.formatProfile;
    if (!profile || typeof profile !== "object") {
      throw new Error(`Canonical standard surface ${surface.id} must define formatProfile.`);
    }
    for (const [field, collection] of Object.entries(profileCollections)) {
      if (!profile[field] || !collection?.[profile[field]]) {
        throw new Error(`Canonical standard surface ${surface.id} has an unknown ${field} profile.`);
      }
    }
  }
  if (Object.keys(grammar.layoutArchetypes || {}).some((id) => /four-column/u.test(id))) {
    throw new Error("Issue #2226 Phase 2 must not invent a shell-level four-column layout archetype.");
  }
