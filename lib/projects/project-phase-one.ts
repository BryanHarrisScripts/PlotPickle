...
import { createProjectEntityId, currentProjectTimestamp, projectContentFingerprint } from "./persistence/project-fingerprints";
...
const makeId = createProjectEntityId;
const timestamp = currentProjectTimestamp;
...
export function addSourceAttribution(project: PhaseOneProject, input: Omit<SourceAttribution, "id" | "createdAt">): PhaseOneProject {
  const record: SourceAttribution = { ...input, id: makeId("attribution"), createdAt: new Date().toISOString() };
...
export function addAiProvenance(project: PhaseOneProject, input: Omit<AiProvenanceRecord, "id" | "createdAt">): PhaseOneProject {
  const record: AiProvenanceRecord = { ...input, id: makeId("ai-provenance"), createdAt: new Date().toISOString() };
