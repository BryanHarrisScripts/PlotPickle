...
const SURFACE_DECLARATIONS = Object.freeze(JSON.parse(readFileSync(
  new URL("../../config/skin-v1-surface-declarations/standard-surfaces.json", import.meta.url),
  "utf8",
)).surfaces || {});
const COLOR_TOKENS = [
...
async function collectProfile(page, surface) {
  ...
  return collectRenderedSurfaceProfile(page, {
    surfaceId: surface,
...
async function navigateAndCollect(page, surface, server) {
...
export async function runSkinV1VisualDirector({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    profiles.dashboard = await navigateAndCollect(page, "dashboard", server);
    geometryFindings.dashboard = analyzeRenderedGeometry(profiles.dashboard, SURFACE_DECLARATIONS.dashboard);
    geometryEvidence.dashboard = await writeRenderedGeometryEvidence({
      page,
      surfaceId: "dashboard",
      evidenceStem: WEBMCP_STANDARD_SURFACE_REGISTRY.dashboard.evidenceStem,
      profile: profiles.dashboard,
      declaration: SURFACE_DECLARATIONS.dashboard,
      findings: geometryFindings.dashboard,
    });
    ...
    for (const surface of SURFACES.slice(1)) {
      profiles[surface] = await navigateAndCollect(page, surface, server);
      geometryFindings[surface] = analyzeRenderedGeometry(profiles[surface], SURFACE_DECLARATIONS[surface]);
      geometryEvidence[surface] = await writeRenderedGeometryEvidence({
        page,
        surfaceId: surface,
        evidenceStem: WEBMCP_STANDARD_SURFACE_REGISTRY[surface].evidenceStem,
        profile: profiles[surface],
        declaration: SURFACE_DECLARATIONS[surface],
        findings: geometryFindings[surface],
      });
    ...
    await writeRenderedGeometrySummary({ profiles, findingsBySurface: geometryFindings, evidenceBySurface: geometryEvidence });
    const dashboard = profiles.dashboard;
    const peerProfiles = Object.values(profiles);
    for (const surface of SURFACES.slice(1)) {
      const findings = analyzeVisualContinuity(dashboard, profiles[surface], peerProfiles);
      surfaces[surface] = {
        blockers: findings.filter((item) => item.severity === "blocker").length,
        advisories: findings.filter((item) => item.severity === "advisory").length,
        findings,
        geometry: {
          policy: "diagnostic-observe-before-enforce",
          blockers: (geometryFindings[surface] || []).filter((item) => item.severity === "blocker").length,
          advisories: (geometryFindings[surface] || []).filter((item) => item.severity === "advisory").length,
          findings: geometryFindings[surface] || [],
          evidence: geometryEvidence[surface] || null,
        },
      };
    }
  } finally {
  ...
  const report = {
    ...
    geometryPolicy: "diagnostic-observe-before-enforce",
    geometryEvidenceRoot: ".artifacts/browser-diagnostics/skin-v1-visual-director/geometry",
    totals: {
      ...
      geometryBlockers: Object.values(geometryFindings).flat().filter((item) => item.severity === "blocker").length,
      geometryAdvisories: Object.values(geometryFindings).flat().filter((item) => item.severity === "advisory").length,
    },
    surfaces,
  };
  ...
  console.log(`Rendered geometry diagnostics: .artifacts/browser-diagnostics/skin-v1-visual-director/geometry`);
