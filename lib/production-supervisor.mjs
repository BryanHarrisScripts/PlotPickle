...
function assetClassificationText(asset) {
  return `${asset?.type || ""} ${asset?.kind || ""} ${asset?.role || ""} ${asset?.title || ""} ${asset?.label || ""} ${asset?.extensions?.role || ""}`;
}
...
export function auditProjectCoverage(projectInput = {}) {
  ...
  const videoAssets = projectAssets.filter((asset) => /video|animatic|trailer/i.test(assetClassificationText(asset)));
  const graphicNovelAssets = projectAssets.filter((asset) => /graphic|panel|page|comic/i.test(assetClassificationText(asset)));
  const posterAssets = projectAssets.filter((asset) => /poster|key.?art/i.test(assetClassificationText(asset)));
