...
function childSurfaces(surface, registry) {
  return registry.surfaces
    .filter((candidate) => candidate.parent === surface.id && candidate.capturePolicy === "standard")
    .sort((left, right) => {
...
function menuContract(surface, declaration, registry) {
  ...
  const family = declaredMenuRegion?.role || (children.length ? "directory" : inherited ? "inherited" : "none");
  ...
  if (family !== "none") {
    ...
    else if (children.length) orientation = "profile-defined";
  ...
  return {
    ...
    expectedVisibleLabels: children.map((candidate) => candidate.navigationPath?.at(-1)?.label || candidate.label),
  };
}
...
export function resolveSkinV1SurfaceContracts(sources) {
  const { registry, grammar, composition, declarations, sceneWorkspace } = sources;
