import type { PlotPickleProject, ReviewThread } from "../../lib/projects/project";
import type { FeedbackTargetReference } from "../../lib/unified-feedback";
import { createFeedback } from "../../lib/unified-feedback-store";
...
function decode(thread: ReviewThread): WritersRoomSession | null {
  ...
  let parsed: unknown;
  try {
    parsed = JSON.parse(comment.body.slice(PREFIX.length));
  } catch (error) {
    if (!(error instanceof SyntaxError)) throw error;
    parsed = null;
  }
  const value = parsed as WritersRoomSession | null;
