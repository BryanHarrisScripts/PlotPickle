export const CANONICAL_NAVIGATION = [
  { id: "dashboard", label: "Dashboard", detail: "Start" },
  { id: "community", label: "Community", detail: "Guildhall" },
  { id: "wyrmwood", label: "Wyrmwood", detail: "Game" },
  { id: "learn", label: "Learn", detail: "Guides" },
  { id: "plan", label: "Plan", detail: "Design" },
  { id: "build", label: "Build", detail: "Assemble" },
  { id: "storyboard", label: "Storyboard", detail: "Sketch" },
  { id: "graphic-novel", label: "Previs", detail: "Visualize" },
  { id: "write", label: "Write", detail: "Draft" },
  { id: "edit", label: "Edit", detail: "Polish" },
  { id: "feedback", label: "Feedback", detail: "Review" },
  { id: "refine", label: "Refine", detail: "Decide" },
  { id: "reports", label: "Reports", detail: "Deliver" },
  { id: "settings", label: "Settings", detail: "Config" },
];

export const CANONICAL_NAVIGATION_IDS = CANONICAL_NAVIGATION.map((item) => item.id);
export const CANONICAL_NAVIGATION_LABELS = CANONICAL_NAVIGATION.map((item) => item.label);
export const CANONICAL_NAVIGATION_GAPS = [
...
function navigationIds(navigation) {
  return (navigation || []).map((entry) => {
    if (entry && typeof entry === "object") return clean(entry.id);
    const label = clean(entry);
    return CANONICAL_NAVIGATION.find((item) => item.label.toLowerCase() === label.toLowerCase())?.id || label.toLowerCase();
  }).filter(Boolean);
}

export function auditNavigationContract(snapshot) {
  const findings = [];
  const actualIds = navigationIds(snapshot?.navigation);
  if (actualIds.join("|") !== CANONICAL_NAVIGATION_IDS.join("|")) {
    findings.push(issue(
      "navigation-order",
      `Shared navigation order is ${actualIds.join(" → ") || "missing"}; expected ${CANONICAL_NAVIGATION_IDS.join(" → ")}.`,
      "error",
    ));
  }

  const actualGaps = (snapshot?.navigationGaps || [])
    .map((gap) => `${clean(gap?.after)}:${clean(gap?.kind)}`)
    .filter(Boolean);
  const expectedGaps = CANONICAL_NAVIGATION_GAPS.map((gap) => `${gap.after}:${gap.kind}`);
  if (actualGaps.join("|") !== expectedGaps.join("|")) {
    findings.push(issue(
      "navigation-groups",
      `Shared navigation grouping is ${actualGaps.join(", ") || "missing"}; expected ${expectedGaps.join(", ")}.`,
      "error",
    ));
  }
  return findings;
