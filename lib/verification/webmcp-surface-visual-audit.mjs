...
export const WEBMCP_SURFACE_READINESS = Object.freeze({
  dashboard: Object.freeze({
    surface: "DASHBOARD",
    selector: DASHBOARD_CANONICAL_SELECTOR,
    timeout: 15_000,
  }),
  community: Object.freeze({
    surface: "COMMUNITY",
    selector: "section[aria-label='PlotPickle Community'] .pp-skin-v1-community",
    timeout: 20_000,
  }),
  settings: Object.freeze({
    surface: "SETTINGS",
    selector: "section[aria-label='Settings menu'][data-settings-menu='keyboard-directory']",
    timeout: 15_000,
  }),
  "cloud-story-mode": Object.freeze({
    surface: "CLOUD_STORY_MODE",
    selector: "section[aria-label='Cloud Story Mode setup'] [data-skin-v1-cloud-story-mode='true']",
    timeout: 20_000,
  }),
  agents: Object.freeze({
    surface: "AGENTS",
    selector: "section[aria-label='PlotPickle Agents setup'] [data-skin-v1-plotpickle-agents='true']",
    timeout: 30_000,
  }),
  profile: Object.freeze({
    surface: "PROFILE",
    selector: "section[aria-label='User Profile'].pp-skin-v1-profile-surface",
    timeout: 15_000,
  }),
  "local-ai": Object.freeze({
    surface: "LOCAL_AI",
    selector: "section[aria-label='Local Story Mode setup'] [data-skin-v1-local-ai='true'], section[aria-label='Local AI setup'] [data-skin-v1-local-ai='true']",
    timeout: 30_000,
  }),
  node: Object.freeze({
    surface: "NODE",
    selector: "section[aria-label='Node information'] [data-skin-v1-node='true'][data-node-readiness='ready']",
    timeout: 30_000,
  }),
});
...
async function installTestWebMcp(page, toolRoot) {
  ...
  await page.evaluate(async ({ names, allowedTargets }) => {
    ...
    const currentSurface = () => {
      ...
      if (visible(document.querySelector("section[aria-label='User Profile'].pp-skin-v1-profile-surface"))) return "PROFILE";
    ...
    const surfaceScope = (surface = currentSurface()) => {
      ...
      if (surface === "PROFILE") return document.querySelector("section[aria-label='User Profile'].pp-skin-v1-profile-surface");
    ...
    const openTarget = (target) => {
      ...
      if (normalized === "local-ai") {
        if (currentSurface() !== "SETTINGS") throw new Error("Local Story Mode can only be opened from the visible Settings menu.");
        clickSelector("[data-settings-secondary-item='local-story-mode']", "Local Story Mode Settings row");
        return;
      }
      if (normalized === "node") {
        if (currentSurface() !== "SETTINGS") throw new Error("Node can only be opened from the visible Settings menu.");
        clickSelector("[data-settings-secondary-item='node-info']", "Node Info Settings row");
      }
    };
    ...
    const computedPresentation = (node) => {
      ...
      return {
        ...
        directoryRow: node.hasAttribute("data-dashboard-menu-item") || node.hasAttribute("data-settings-secondary-item"),
...
export function visualViolations(contract, baselineTokens = {}) {
  ...
  for (const item of inventory) {
    ...
    if (presentation.directoryRow && expectedControlHeight !== null) {
      const actualHeight = numericPx(presentation.height);
      const maximumDirectoryRowHeight = Math.max(expectedControlHeight * 2.5, expectedControlHeight + 32);
      if (actualHeight !== null && actualHeight > maximumDirectoryRowHeight + 0.5) {
        add(item, "rowHeight", presentation.height, `<= ${maximumDirectoryRowHeight}px`, "layout:directory-row");
      }
    }
  }
...
async function inspectDashboard(page, failures) {
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      brand: brandStyle && { fontFamily: brandStyle.fontFamily, fontSize: brandStyle.fontSize, fontWeight: brandStyle.fontWeight },
      meta: metaStyle && { color: metaStyle.color, fontFamily: metaStyle.fontFamily, fontSize: metaStyle.fontSize, fontWeight: metaStyle.fontWeight },
  ...
  assert(Boolean(
    result.brand?.fontFamily
    && result.meta?.fontFamily
    && result.brand.fontFamily === result.meta.fontFamily
    && Number.parseFloat(result.brand.fontSize || "0") > Number.parseFloat(result.meta.fontSize || "0")
  ), "Dashboard no longer demonstrates unified JetBrains Mono with distinct semantic brand hierarchy.", failures);
...
function readinessViolation(surface) {
  const readiness = WEBMCP_SURFACE_READINESS[surface];
  return {
    surface: readiness?.surface || String(surface || "UNKNOWN").toUpperCase(),
    semanticRole: "surface",
    stableIdentity: String(surface || "unknown"),
    property: "readiness",
    actual: "not-ready",
    expected: "ready",
...
async function waitForSurfaceReady(page, surface) {
  const readiness = WEBMCP_SURFACE_READINESS[surface];
  if (!readiness) throw new Error(`No WebMCP readiness contract exists for ${surface}.`);
  const ready = await page.locator(readiness.selector).first()
    .waitFor({ state: "visible", timeout: readiness.timeout })
    .then(() => true, () => false);
  if (!ready) throw new Error(`UI-CONFORMANCE ${JSON.stringify(readinessViolation(surface))}`);
}
...
export async function runWebMcpSurfaceVisualAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  const browserSession = await createBrowserVerificationSession({
    toolRoot,
    runId: "webmcp-surface-visual-audit",
  ...
  const inspectCurrent = async (surface) => {
    ...
    await browserSession.checkpoint(page, {
      surfaceId: surface,
      route: page.url(),
      actionId: "inspect-surface-visual-contract",
  ...
  try {
    ...
    await waitForSurfaceReady(page, "dashboard");
    ...
    await waitForSurfaceReady(page, "settings");
    const settingsAvailable = await executeTool(page, "list_available_surfaces");
    const settingsRows = Array.isArray(settingsAvailable?.settings) ? settingsAvailable.settings : [];
    assert(settingsRows.map((item) => item.id).join(",") === "general,local,cloud,hybrid,node-info,agents,ai-routing,buzz-settings", "WebMCP Settings discovery did not match the #2320 flattened Settings directory.", failures);
    assert(settingsRows.length === 8 && settingsRows.every((item) => item.connected), "WebMCP Settings discovery did not expose all eight #2320 Settings destinations as connected.", failures);
  ...
  // Visual candidates remain intentionally bounded to Dashboard, Community, Settings and Profile.
  // Story Mode child Local/Cloud surfaces remain exercised by the Settings/menu contract UAT.

  if (failures.length) throw new Error(`WebMCP Surface Visual UAT failed:\n- ${failures.join("\n- ")}`);
  const locked = WEBMCP_ALLOWED_TARGETS.filter((surface) => manifest.surfaces[surface]?.status === "locked").length;
  const inspected = conformance.reduce((sum, entry) => sum + entry.inspected, 0);
  console.log(`WebMCP Surface Visual UAT passed. Dashboard is the canonical Skin V1 design reference. Captured ${WEBMCP_ALLOWED_TARGETS.length} surface candidates; ${locked} locked visual baselines enforced.`);
