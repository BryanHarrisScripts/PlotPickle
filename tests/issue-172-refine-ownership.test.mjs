...
test("issue #172 preserves the ten-workspace sequence and adds owner-aware entry points", async () => {
  ...
  const orderedLabels = ["Dashboard", "Learn", "Plan", "Storyboard", "Write", "Graphic Novel", "Build", "Feedback", "Refine", "Reports"];
  let cursor = -1;
  ...
  for (const workspace of ["learn", "plan", "storyboard", "write", "pitch", "build", "feedback", "refine", "reports"]) assert.ok(page.includes(`${workspace}:`), `Missing query route for ${workspace}`);
  assert.match(page, /<WorkspaceCapabilityShelf workspace=\{capabilityOwner\}/);
  for (const owner of ["Learn", "Plan", "Storyboard", "Write", "Pitch", "Build", "Feedback", "Reports"]) assert.ok(shelf.includes(`${owner} owns`) || shelf.includes(`${owner} · read only`), `Missing owner shelf for ${owner}`);
});
...
test("Storyboard and Build divide production tools and Reports remains read-only", async () => {
  ...
  for (const storyboardView of ['id: "shots"', 'id: "animatic"']) assert.match(production, new RegExp(`${storyboardView.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[\\s\\S]*owner: "storyboard"`));
  for (const buildView of ['id: "sonic"', 'id: "breakdowns"', 'id: "shoot-groups"', 'id: "schedule"', 'id: "distribution"']) assert.match(production, new RegExp(`${buildView.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[\\s\\S]*owner: "build"`));
...
test("specialist labs use owner scopes and require a Feedback approval handoff", async () => {
  const [labs, route] = await Promise.all([source("app/specialist-labs.tsx"), source("app/labs/page.tsx")]);
  for (const contract of ['{ id: "dialogue", label: "Dialogue Lab"', 'owner: "refine"', '{ id: "research", label: "Research & Canon"', 'owner: "plan"', '{ id: "visual", label: "Visual Bible"', 'owner: "storyboard"', '{ id: "passes", label: "Saved-Pass Approval History"', 'owner: "feedback"']) assert.ok(labs.includes(contract), `Missing lab ownership contract: ${contract}`);
...
test("Learn Plan and Refine divide Story Craft Essentials without duplicate editors", async () => {
  const [craft, voice, resonance, pageflow, draftlens] = await Promise.all([source("app/story-craft-essentials/page.tsx"), source("app/voiceprint/page.tsx"), source("app/resonance/page.tsx"), source("app/pageflow/page.tsx"), source("app/draftlens/page.tsx")]);
...
test("Refine has exactly seven diagnostic passes and no moved editor card", async () => {
  ...
  const titles = ["Overview & Diagnostic Queue", "Structure & Pacing Diagnostics", "Story & Theme through Resonance", "Character & Dialogue Diagnostics", "Page & Scene Diagnostics through PageFlow", "Full-Draft Diagnosis through DraftLens", "Revision Passes & Essential Craft Audit"];
  for (const title of titles) assert.ok(hub.includes(title), `Missing Refine pass: ${title}`);
  assert.equal((hub.match(/code: "/g) ?? []).length, 7);
  for (const moved of ["Pitch & Review Studio", "Production Studio", "Specialist Labs", "Voiceprint Engine", "CraftLoop Engine"]) assert.ok(!hub.includes(`title: "${moved}"`), `Refine still owns moved editor: ${moved}`);
...
test("Reports adds provenance without a second database or mutation path", async () => {
  const [model, workspace] = await Promise.all([source("lib/consolidated-reports.ts"), source("app/reports-workspace.tsx")]);
...
test("issue #172 documentation and focused test are registered", async () => {
  const [documentation, packageText] = await Promise.all([source("docs/issue-172-refine-ownership.md"), source("package.json")]);
  for (const phrase of ["Refine diagnoses and proposes.", "Feedback owns discussion and approval.", "Reports owns read-only summaries.", "Dashboard, Learn, Plan, Storyboard, Write, Pitch, Build, Feedback, Refine, Reports."]) assert.ok(documentation.includes(phrase), `Missing #172 documentation contract: ${phrase}`);
