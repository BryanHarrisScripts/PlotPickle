...
export function skinV1RouteMigrationLedger() {
  ...
  const routedCompatibilityDebt = SKIN_V1_SURFACES
    .filter((surface) => surface.capturePolicy === "census-only" && typeof surface.route === "string" && surface.route)
    .map((surface) => ({
      id: surface.id,
      route: surface.route,
  ...
  const stateCompatibilityDebt = SKIN_V1_SURFACES
    .filter((surface) => surface.capturePolicy === "census-only" && !surface.route)
    .map((surface) => ({
      id: surface.id,
