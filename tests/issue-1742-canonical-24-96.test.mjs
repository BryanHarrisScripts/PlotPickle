...
function acceptStage(stage, stamp) {
  stage.content = `${stage.content || "Accepted writer material"}`;
  stage.state = "accepted";
  stage.updatedAt = stamp;
  stage.acceptedAt = stamp;
}

test("#1742 creates exactly 24 Blocks and 96 canonical mini-blocks", async () => {
  ...
  for (const mini of structure.blocks[0].miniBlocks) {
    ...
    assert.equal(mini.stages.write.state, "locked");
...
test("#1742 unlocks stages and the next Block only from explicit accepted evidence", async () => {
  const contract = await structureModule();
  ...
  for (const mini of sourceStructure.blocks[0].miniBlocks) {
    for (const stageName of ["plan", "build", "write", "storyboard"]) {
