...
test("Phase 4 produces deterministic story health checks", () => {
  for (const check of ["catalyst", "ghost", "empty-blocks", "unused-characters", "orphan-frames"]) assert.match(engine, new RegExp(`id: "${check}"`));
  assert.match(engine, /score = Math\.max/);
});

test("dependency artifacts remain written into the modular project", () => {
  for (const file of ["dependencies/graph.json", "dependencies/references.json", "dependencies/reverse-index.json", "dependencies/conflicts.json", "dependencies/health.json", "reports/story-health.json"]) assert.match(folder, new RegExp(file.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
...
test("Phase 2 through Phase 4 project folders remain readable", () => {
  for (const version of ["2.0.0", "2.1.0", "2.2.0"]) assert.match(folder, new RegExp(version.replaceAll(".", "\\.")));
});
