...
import "./webmcp-canonical-surface-registry.mjs";
...
export async function waitForUiServer(server, timeoutMs = 90_000) {
...
export async function runUiAxeAudit({ serverUrl, toolRoot, routesPath = defaultRoutes, storageStatePath = "" } = {}) {
  ...
  const { AxeBuilder } = await loadTooling(toolRoot);
  ...
  try {
    ...
    for (const route of registry.routes) {
      ...
      await browserSession.checkpoint(page, {
        surfaceId: route.id || route.label || route.path,
        route: url,
        actionId: "axe-audit",
