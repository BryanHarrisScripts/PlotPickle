...
function structuredFoundationPass(value: string) {
  ...
  const candidate = root.values && typeof root.values === "object" && !Array.isArray(root.values)
    ...
    : root;
  return ["output-1", "output-2"].every((fieldId) => typeof candidate[fieldId] === "string" && Boolean((candidate[fieldId] as string).trim()));
}
...
async function loadThemeProbe() {
  ...
  const context = [
    lesson?.title,
    lesson?.overview,
    themeDefinition ? `Theme: ${themeDefinition.meaning}` : "",
...
async function runFoundationsProbe(baseUrl: string) {
  ...
  return { latencyMs: result.latencyMs || Date.now() - started, structured: structuredFoundationPass(text) };
}
...
export async function runStartupAgentDiagnostics(baseUrl: string) {
  ...
  if (fastReady && sageRegistered) {
    try {
      ...
      const antiEchoState = sage.antiEcho ? "PASS" : responsePass && sage.repetitionSafe ? "WARN" : "FAIL";
      const groundingState = sage.grounded ? "PASS" : responsePass && sage.repetitionSafe ? "WARN" : "FAIL";
      printResult("Sage anti-echo check", antiEchoState, sage.antiEcho ? recoveryDetail : "raw startup probe; visible Sage still applies its response repair guard");
      printResult("Sage repetition guard", sage.repetitionSafe ? "PASS" : "FAIL", sage.repetitionSafe ? recoveryDetail : "runaway phrase repetition detected");
      printResult("Curriculum grounding", groundingState, sage.grounded ? recoveryDetail : "raw probe did not preserve enough curriculum meaning; visible Sage still validates before display");
      failed ||= !responsePass || !sage.repetitionSafe;
      warned ||= !sage.antiEcho || !sage.grounded;
    } catch (error) {
...
export function startupAgentDiagnosticsPlugin(): Plugin {
  return {
    name: "plotpickle-startup-agent-diagnostics-v2",
