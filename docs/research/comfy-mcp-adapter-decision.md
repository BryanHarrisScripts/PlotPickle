...
- configured workflow-node checks;
- normalized GPU name, total VRAM and free VRAM;
- plain-language Settings visibility for management, service and GPU readiness;
- actionable setup blockers for stopped service, missing checkpoints and missing required nodes;
...
Do **not** add a second MCP workflow-execution bridge in #1083.

The direct ComfyUI API remains the execution path because it already satisfies PlotPickle's bounded request, continuity, artifact and provenance contracts. Revisit MCP execution only if it later solves a measured reliability or capability gap.
