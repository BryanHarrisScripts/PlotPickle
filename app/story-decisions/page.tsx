...
export default function StoryDecisionsPage() {
  ...
  async function respond(responseClass: string, extra: Record<string, unknown> = {}) {
    ...
    try {
      ...
      setNotice(responseClass === "defer" ? "Decision deferred. It remains in your Story Decision channel." : responseClass === "request-alternatives" ? "Alternative request recorded for the workflow." : "Decision recorded. Open Story Workbench to validate the exact current PPF target before any canon change.");
      await refresh();
  ...
  return <main className={styles.page}>
    ...
    <div className={styles.layout}>
      ...
      <section className={styles.detail} aria-live="polite">{selected ? <>
        ...
        <section><h3>Your response</h3><div className={styles.actions}>{selected.status === "answered" ? <Link href={`/story-workbench?decisionId=${encodeURIComponent(selected.decisionId)}`}>Open Story Workbench</Link> : null}<button disabled={busy || selected.status === "answered" || !selected.proposedChange || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("accept-proposal")}>Accept recommendation</button><button disabled={busy || selected.status === "answered" || !selected.proposedChange || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("reject-proposal")}>Reject recommendation</button><button disabled={busy || selected.status === "answered" || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("keep-current")}>Keep current story</button><button disabled={busy || selected.status === "answered" || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("request-alternatives")}>Ask for alternatives</button><button disabled={busy || selected.status === "answered"} onClick={() => void respond("defer")}>Defer</button><Link href="/?workspace=build">Open in story</Link></div>
          ...
          <button className={styles.primary} disabled={busy || selected.status === "answered" || !replacement.trim() || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond(selected.proposedChange ? "modify-proposal" : "freeform-decision")}>{busy ? "Recording…" : "Send to Story Workbench"}</button>
        </section>
