...
async function waitForSurfaceReady(page, surface) {
  ...
  try {
    await page.locator(readiness.selector).first().waitFor({ state: "visible", timeout: readiness.timeout });
  } catch {
    throw new Error(`UI-CONFORMANCE ${JSON.stringify(readinessViolation(surface))}`);
  }
}
