...
test("Settings exposes one-click Guildhall setup while Profile owns the Human signer", () => {
  ...
  assert.match(settings, /Profile-owned signing key/);
  assert.match(settings, /Settings never asks for the nsec/);
...
test("Settings preserves BUZZ owner review for Orin and Fen", () => {
  ...
  assert.match(settings, /PlotPickle does not bypass owner review/);
