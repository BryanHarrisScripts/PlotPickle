...
async function capture(page, manifest, surface, failures) {
  ...
  const orchestrated = page.locator(`${ORCHESTRATOR_SELECTOR} [data-skin-v1-orchestrated="true"][data-skin-v1-orchestrator-active="true"][data-skin-v1-surface-id="${surface}"]`).first();
  try {
  ...
  const candidate = await frame.screenshot({ path: candidatePath, animations: "disabled" });
...
export async function runWebMcpStandardSurfaceCatalogue({ serverUrl, toolRoot, storageStatePath = "", onSurface = null } = {}) {
  ...
  try {
    ...
    for (const surface of WEBMCP_STANDARD_SURFACE_TARGETS) {
      ...
      try {
        await page.locator(contract.readySelector).first().waitFor({ state: "visible", timeout: contract.timeout });
        await page.locator(`${ORCHESTRATOR_SELECTOR} [data-skin-v1-surface-id="${surface}"][data-skin-v1-orchestrator-active="true"]`).first().waitFor({ state: "visible", timeout: contract.timeout });
      } catch {
