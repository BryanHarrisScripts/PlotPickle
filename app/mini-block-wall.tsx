...
function toneIndex(card: MiniBlockWallCard, state: MiniBlockWallState) {
  ...
  const match = state.customLabel.trim().toLocaleLowerCase();
  if (match) {
    const text = `${card.label} ${card.function} ${card.purpose}`.toLocaleLowerCase();
    return text.includes(match) ? 6 : 0;
  }
  return hash(card.label || "unlabelled") % 8;
}

function expanded(card: MiniBlockWallCard, state: MiniBlockWallState, anchor: MiniBlockWallCard) {
...
function legendValues(project: PlotPickleProject, state: MiniBlockWallState, cards: MiniBlockWallCard[]) {
  if (state.colourMode === "status") return STATUS_OPTIONS.filter((option) => option.id).map((option) => option.label);
  if (state.colourMode === "character") return project.characters.map((character) => character.name || "Unnamed character");
  if (state.colourMode === "storyline") return project.storyThreads.map((thread) => thread.name);
  if (state.colourMode === "location") return project.world.locations.map((location) => location.name);
  if (state.colourMode === "setup-payoff") return ["Neither", "Setup", "Payoff", "Both"];
  return [...new Set(cards.map((card) => card.label).filter(Boolean))];
}
...
export default function MiniBlockWall({ project, onProjectChange, onOpenBlock }: MiniBlockWallProps) {
  ...
  const legend = legendValues(project, state, model.cards).filter(Boolean).slice(0, 8);
  ...
  function selectCard(card: MiniBlockWallCard) {
    updateState({
  ...
  function moveSelection(offset: number) {
    ...
    const nextIndex = Math.min(model.visibleCards.length - 1, Math.max(0, (visibleIndex < 0 ? 0 : visibleIndex) + offset));
    selectCard(model.visibleCards[nextIndex]);
  }
  ...
  return (
    <section className={styles.wallWorkspace} aria-label="96 mini-block whole-film wall">
      ...
      <section className={styles.expansionBar} aria-label="Expansion zoom and pan controls">
        <div role="group" aria-label="Expand mini-block scope">
          {(["all", "act", "sequence", "block"] as MiniBlockWallScope[]).map((scope) => <button type="button" key={scope} className={state.expandedScope === scope ? styles.active : ""} onClick={() => updateState({ expandedScope: scope })}>{scope === "all" ? "Expand all 96" : `Expand selected ${scope}`}</button>)}
        </div>
        ...
        <button type="button" onClick={() => updateState({ zoom: 1, pan: { x: 0, y: 0 }, filters: DEFAULT_MINI_BLOCK_WALL_STATE.filters })}>Reset board</button>
      </section>
      ...
      <div className={styles.legend} aria-label={`${state.colourMode} colour legend`}>
        ...
        {legend.length ? legend.map((item, index) => <span key={item}><i className={styles[`tone${index % 8}`]} />{item}</span>) : <span>No labelled items yet.</span>}
      </div>
      ...
      <div className={styles.boardLayout}>
                    ...
                    {actBlocks.map((block) => {
                      ...
                      return (
                        <section className={styles.blockGroup} key={block.id}>
                          <header><div><span>Block {block.number} · Sequence {block.sequenceNumber}</span><strong>{block.title}</strong></div><small>{cards.length}/4 visible</small></header>
                          <div className={styles.miniGrid}>
                            {cards.map((card) => {
                              const isExpanded = anchor ? expanded(card, state, anchor) : true;
                              const warningCount = warningsByCard.get(card.id) ?? 0;
                              return (
                                <button
                                  type="button"
                                  key={card.id}
                                  className={`${styles.miniCard} ${styles[`tone${toneIndex(card, state)}`]} ${card.id === selectedCard?.id ? styles.selected : ""} ${isExpanded ? "" : styles.compact}`}
                                  ...
                                  onKeyDown={(event) => {
                                    ...
                                    if (event.key === "Home") { event.preventDefault(); selectCard(model.visibleCards[0]); }
                                    if (event.key === "End") { event.preventDefault(); selectCard(model.visibleCards[model.visibleCards.length - 1]); }
                                  }}
                                  aria-pressed={card.id === selectedCard?.id}
                                >
                                  <span className={styles.cardTop}><strong>{card.globalNumber}</strong><i>{labelForStatus(card.status)}</i>{warningCount ? <b>{warningCount}</b> : null}</span>
        ...
        <aside className={styles.inspector} aria-label="Mini-block inspector">
          {selected && selectedCard ? <>
            <header><div><p>Mini-block inspector</p><h3>{selectedCard.globalNumber} · {selected.miniBlock.label}</h3></div><span>{labelForStatus(selectedCard.status)}</span></header>
