...
export function storyboardPrompt(project: PlotPickleProject, block: StoryBlock, scene: StoryScene, mini: MiniBlock, frame: VisualFrame) {
  ...
  const characters = characterIds.map((id) => project.characters.find((character) => character.id === id)).filter(Boolean);
  ...
  const characterText = characters.map((character) => [
    ...
    character!.description && `appearance and identity: ${character!.description}`,
    character!.want && `immediate bearing shaped by wanting ${character!.want}`,
    character!.need && `deeper need ${character!.need}`,
    character!.ghost && `emotional history ${character!.ghost}`,
    character!.fatalFlaw && `flaw ${character!.fatalFlaw}`,
    character!.strengths && `strengths ${character!.strengths}`,
    character!.arc && `arc ${character!.arc}`,
    character!.image && "match the established character reference portrait",
  ...
  return [
    ...
    characterText && `Characters present; preserve these identities consistently across every frame: ${characterText}.`,
    ...
    "Keep wardrobe, age, facial structure, hair, props, injuries, time of day, colour language, and screen direction consistent with established project details. No captions, lettering, logos, borders, watermarks, or UI.",
  ].filter(Boolean).join("\n");
}
...
export default function VisualStoryboard({
  ...
  async function refinePrompt() {
    ...
    setWorking("prompt");
    setMessage("Refining the prompt from the current story context…");
    try {
      const response = await fetch("/api/local-ai/generate/text", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          instructions: "Rewrite the supplied storyboard image prompt for an image model. Preserve every story, character, location, continuity, and camera fact. Strengthen visible action, composition, lighting, spatial relationships, and emotional readability. Return only the final image prompt without Markdown.",
  ...
  async function generateImage() {
    ...
    setWorking("image");
    setMessage("Generating the storyboard frame and saving it on this computer…");
    try {
      const response = await fetch("/api/local-ai/generate/image", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: activePrompt, assetId: `storyboard-block-${block.number}-mini-${miniBlockNumber}`, aspect: "landscape" }),
      });
      const result = await response.json() as AiResponse;
      ...
      setWorking("idle");
      setMessage("Storyboard frame generated, saved locally, and attached to this mini-block.");
    } catch (error) {
      setWorking("error");
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div><span>Visual Storyboard</span><h1>Put the entire movie in front of you.</h1><p>Move from the 24-block overview into all 96 mini-block visuals. PlotPickle builds each prompt from the story you have already developed.</p></div>
      ...
      <div className={styles.workspace}>
        ...
        <aside className={styles.inspector}>
          ...
          <p className={styles.note}>AI is optional. The generated image is saved by the private local server; the prompt can also be copied to another image tool.</p>
        </aside>
      </div>
    </div>
  );
