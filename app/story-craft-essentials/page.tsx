...
export default function StoryCraftEssentialsPage() {
  ...
  const [formatTemplateId, setFormatTemplateId] = useState(advancedFormattingTemplates[0].id);
  const [formatText, setFormatText] = useState(advancedFormattingTemplates[0].form);
  ...
  return <main className={styles.page}>
    <header className={styles.hero}><div><span>Why → how → evidence → diagnose → revise</span><h1>Story Craft Essentials</h1><p>Connect audience experience, pacing, tone, theme, scene movement, screen evidence, motifs, advanced screenplay forms and revision priorities to the active PlotPickle project.</p></div><nav><Link href="/">Back to PlotPickle</Link><Link href="/read-learn">Read & Learn</Link><Link href="/draftlens">DraftLens</Link></nav></header>
