...
CraftLoop, DraftLens and Resonance now sit under Refine as canonical orchestrated census routes while preserving their existing feature owners. Their single visible Skin V1 Return delegates to the feature-owned Refine return, and CraftLoop still prefers its more-specific PRE-PRODUCTION return when launched from Outline, Storyboard or Previs context. The old `/?workspace=refine` links on these surfaces are retired in favor of canonical `/diagnostics`.
