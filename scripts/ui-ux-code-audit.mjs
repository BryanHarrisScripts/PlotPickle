...
function contradictsPlatformStandards(finding, source, context) {
  ...
  if (finding.criterion === 11 && /<div\b/i.test(evidence) && /(?:replace[^.]*button|add\s+role=["']?button|make it accessible|button functionality|use\s+(?:a\s+)?button(?:\s+element)?|interactive controls?)/i.test(combined) && !/\b(?:onClick|onKeyDown|onKeyUp|onPointerDown|role=["']button["']|tabIndex)\b/i.test(evidence)) return true;
