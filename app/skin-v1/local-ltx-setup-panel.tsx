  ...
  return (
    <section style={panel} aria-labelledby="local-ltx-title">
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 14, flexWrap: "wrap" }}>
        <div>
          <p style={{ margin: 0, color: "var(--pp-skin-accent-bright)", fontSize: 12 }}>LOCAL AI / LTX-VIDEO</p>
          <h2 id="local-ltx-title" style={{ margin: "5px 0 8px" }}>LTX-VIDEO 2B 0.9.8 DISTILLED</h2>
          <p style={{ margin: 0, maxWidth: 880, lineHeight: 1.5, color: "var(--pp-skin-ink-soft)" }}>
        ...
        <span style={{ border: `var(--pp-skin-border-thin) solid ${status?.ready ? "var(--pp-skin-accent-bright)" : "var(--pp-skin-warning-line)"}`, padding: "5px 9px", color: status?.ready ? "var(--pp-skin-accent-bright)" : "var(--pp-skin-warning)" }}>
      ...
      <div style={{ ...card, marginTop: 12, borderColor: status?.ready ? "var(--pp-skin-accent)" : "var(--pp-skin-warning-line)", background: status?.ready ? "var(--pp-skin-surface-2)" : "var(--pp-skin-warning-surface)" }}>
        <p style={{ margin: 0, color: status?.ready ? "var(--pp-skin-accent-bright)" : "var(--pp-skin-warning)", fontSize: 12, letterSpacing: ".08em" }}>{status?.ready ? "ENGINE STATUS" : "SETUP BLOCKER"}</p>
        <h3 style={{ margin: "7px 0" }}>{blocker.title}</h3>
        <p style={{ margin: 0, color: "var(--pp-skin-ink-soft)", lineHeight: 1.5 }}>{installing ? blocker.detail : status?.blockers?.length ? status.blockers.join("; ") : blocker.detail}</p>
        {status?.missingModels.length ? <p style={{ margin: "8px 0 0", color: "var(--pp-skin-ink-soft)" }}>SET UP LTX can download and SHA-256 verify the reviewed files after one explicit approval. Maximum download: {status.reviewedDownloadSize || "16.13 GB"}.</p> : null}
        <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
          <button type="button" style={warningButton} onClick={() => void setupDefault()} disabled={working || installing}>{installing ? "INSTALLING LTX MODELS..." : "SET UP LTX"}</button>
          <button type="button" style={warningButton} onClick={() => void refresh()} disabled={working}>{working ? "WORKING..." : "CHECK AGAIN"}</button>
      ...
      <p role="status" aria-live="polite" style={{ margin: "12px 0 0", color: testResult?.error || status?.setupTask?.state === "failed" ? "var(--pp-skin-warning-ink)" : "var(--pp-skin-accent-bright)" }}>{notice}</p>
      {testResult?.outputAssetUrl ? <p style={{ margin: "8px 0 0" }}><a href={testResult.outputAssetUrl} target="_blank" rel="noreferrer" style={{ color: "var(--pp-skin-warning)" }}>OPEN TEST VIDEO</a></p> : null}

      <details open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)} style={{ marginTop: 16, border: "var(--pp-skin-border-thin) solid var(--pp-skin-line)", background: "var(--pp-skin-surface-0)", padding: 12 }}>
        ...
        <textarea
          ...
          style={{ width: "100%", minHeight: 180, boxSizing: "border-box", background: "var(--pp-skin-surface-0)", color: "var(--pp-skin-ink)", border: "var(--pp-skin-border-thin) solid var(--pp-skin-line)", borderRadius: "var(--pp-skin-radius)", padding: 10, font: "inherit" }}
        />
        <div style={{ marginTop: 10 }}>
          <button type="button" style={warningButton} onClick={() => void importManifest()} disabled={working || !manifestText.trim()}>{working ? "WORKING..." : "IMPORT REVIEWED MANIFEST"}</button>
        </div>
      </details>
    </section>
  );
