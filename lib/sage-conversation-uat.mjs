export const SAGE_CONVERSATION_UAT_CASES = Object.freeze([
  ...
  { id: "name-meaning", question: "What does your name mean?", kind: "name-meaning" },
  { id: "wellbeing", question: "Are you ok?", kind: "conversation" },
...
export function assessSageConversationAnswer(testCase, answer) {
  ...
  if (testCase.kind === "name-meaning") {
    if (!/\bsage\b/.test(norm) || !/\bbrinewick\b/.test(norm)) failures.push("Name-meaning response did not explain both parts of Sage Brinewick's name.");
    if (/software mentor here to help you understand the lessons and apply them to your story/.test(norm)) failures.push("Name-meaning response incorrectly repeated the generic identity answer.");
  }
...
export function findingFingerprint(message) {
  ...
  if (text.includes("name meaning response")) return "sage.name-meaning-invalid";
