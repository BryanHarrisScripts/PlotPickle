...
replace("tests/issue-87-navigation-cleanup.test.mjs", String.raw`  assert.match(page, /useState\(false\)/);`, String.raw`  assert.match(page, /useState\(true\)/);`);
...
replace("tests/issue-63-new-writer-path.test.mjs", `  assert.match(welcome, /Simple Start · optional guided entry/);`, String.raw`  assert.match(main, /const \[showLanding, setShowLanding\] = useState\(true\)/);
...
replace("tests/proposed-primary-menu.test.mjs", String.raw`  assert.match(page, /useState<MainTab>\("dashboard"\)/);`, String.raw`  assert.match(page, /useState<MainTab>\("dashboard"\)/);
  assert.match(page, /const \[showLanding, setShowLanding\] = useState\(true\)/);`);
replace("tests/proposed-primary-menu.test.mjs", String.raw`  const css = await source("app/ui-ux-cleanup.css");
  assert.match(css, /Proposed simplified primary menu/);
  assert.match(css, /grid-template-columns: minmax\(220px, 0\.9fr\) minmax\(0, 4fr\)/);
  assert.match(css, /min-height: 64px/);`, String.raw`  const css = await source("app/premium-ui.css");
