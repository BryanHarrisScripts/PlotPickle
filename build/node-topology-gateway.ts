...
function isLoopbackAddress(value: string | undefined) { return value === "127.0.0.1" || value === "::1" || value === "::ffff:127.0.0.1"; }
...
function sendTopologyJson(response: ServerResponse, status: number, body: Record<string, unknown>) {
  response.writeHead(status, { "Content-Type": "application/json; charset=utf-8", "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff" }).end(JSON.stringify(body));
}
function requestOrigin(request: IncomingMessage) { return `http://${request.headers.host || "127.0.0.1:4173"}`; }

export function registerPlotPickleNodeTopologyGateway(server: ViteDevServer) {
  server.middlewares.use((request, response, next) => {
    const pathname = request.url?.split("?", 1)[0] || "";
    if (pathname !== NODE_TOPOLOGY_PATH) { next(); return; }
    if (!isLocalTopologyRequest(request)) {
      sendTopologyJson(response, 403, { ok: false, message: "PlotPickle node topology diagnostics are restricted to this local Studio until hosted authentication and tenant isolation are implemented." });
      return;
    }
    if (request.method !== "GET") { sendTopologyJson(response, 405, { ok: false, message: "Method not allowed." }); return; }
    void (async () => {
      const runtime = await localRuntimeSnapshot();
      const hardware: PlotPickleNodeHardwareSummary = {
        platform: process.platform, architecture: process.arch, cpuModel: runtime.hardware.cpuModel, ramGb: runtime.hardware.ramGb,
        gpuName: runtime.hardware.gpuName, gpuGeneration: runtime.hardware.gpuGeneration, gpuMemoryGb: runtime.hardware.vramGb,
        memoryModel: runtime.hardware.gpuName && runtime.hardware.vramGb > 0 ? "discrete" : "system",
      };
      const currentNode = createLocalDesktopPlotPickleNode({ endpoint: requestOrigin(request), textReady: runtime.activeRuntime.reachable, visionReady: runtime.roles.vision.available, retrievalReady: true, hardware });
    ...
    })().catch((error) => sendTopologyJson(response, 500, { ok: false, message: error instanceof Error ? error.message : "PlotPickle node topology could not be inspected." }));
  });
}
