...
async function inspectRuntimeInvariants(page, contract) {
  return page.locator(contract.rootSelector).first().evaluate((root) => {
    const isRendered = (node, requireBox = true) => {
      if (!(node instanceof HTMLElement)) return false;
      if (requireBox && node.getClientRects().length === 0) return false;
    ...
    const controls = [...root.querySelectorAll("button, a[href], input, select, textarea, [role='button'], [tabindex]")]
      .filter((node) => isRendered(node) && node.getAttribute("aria-hidden") !== "true");
    ...
    const isCanonicalShellReturn = (node) => node.matches?.(".pp-skin-v1-return");
    ...
    const clippedControls = controls
      .filter((node) => {
        if (isCanonicalShellReturn(node) || isInsideHorizontalScroller(node)) return false;
      ...
      .map((node) => {
        ...
        return {
          ...
          accessibleName: accessibleName(node).replace(/\\s+/gu, " ").trim().slice(0, 80),
          rect: { left: round(rect.left), right: round(rect.right) },
          rootRect: { left: round(rootRect.left), right: round(rootRect.right) },
          clippedLeftPx: round(Math.max(0, rootRect.left - rect.left)),
          clippedRightPx: round(Math.max(0, rect.right - rootRect.right)),
        };
      });

    const active = document.activeElement;
    const hiddenFocused = active instanceof HTMLElement && root.contains(active) && !isRendered(active)
    ...
    const pageWidth = documentElement.clientWidth || window.innerWidth;
    const pageScrollWidth = Math.max(documentElement.scrollWidth, document.body?.scrollWidth || 0);
    return {
      duplicateIds,
      ...
      hiddenFocused,
      unresolvedLoading,
      horizontalOverflowPx: Math.max(0, pageScrollWidth - pageWidth),
      rootHorizontalOverflowPx: Math.max(0, root.scrollWidth - root.clientWidth),
