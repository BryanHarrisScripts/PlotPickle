...
export async function runSkinV1MenuContractAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    await page.locator("section[aria-label=\"Writer's Craft menu\"][data-skin-menu='writer-craft']").waitFor({ state: "visible", timeout: 15_000 });
    await inspectMenu(page, "writer-craft", failures);
    const writerFirstRow = page.locator("[data-skin-menu='writer-craft'] [data-skin-menu-row='foundations']");
    await writerFirstRow.focus();
    await page.keyboard.press("ArrowDown");
    await expectSelected(page, "[data-skin-menu='writer-craft'] [data-skin-menu-row='visual-writing']", "WRITER_CRAFT", "writer-craft-visual-writing", failures);
    ...
    await page.locator("section[aria-label='LEARN Journey semester'][data-skin-menu='learn-journey-courses']").waitFor({ state: "visible", timeout: 20_000 });
    await inspectMenu(page, "learn-journey-courses", failures);
    await inspectSolidChrome(page, "LEARN_JOURNEY_COURSES", failures);
    const courseRows = page.locator("[data-skin-menu='learn-journey-courses'] [data-learn-course-status='preview-only']");
    ...
    if (unavailableCourseCount !== 4) {
      addFailure(failures, violation("LEARN_JOURNEY_COURSES", "learn-journey-course-shells", "unavailableCourseCount", unavailableCourseCount, 4));
    }
    ...
    if (availableCourseCount !== 0) {
      addFailure(failures, violation("LEARN_JOURNEY_COURSES", "learn-journey-course-shells", "availableCourseCount", availableCourseCount, 0));
    }
  ...
  return { menus: ["dashboard", "settings", "cloud-story-mode", "local-story-mode", "writer-craft", "learn-journey", "learn-journey-courses"], status: "pass" };
}
