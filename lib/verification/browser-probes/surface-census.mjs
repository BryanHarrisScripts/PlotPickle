...
import {
  ...
  lifecycleForSurface,
} from "./current-matrix-surface-lifecycle.mjs";
...
export function buildSurfaceCensusSummary(records, dashboard = []) {
  ...
  const currentNavigationExpected = records.filter((record) => record.currentNavigationExpected === true);
  const currentNavigationReachable = currentNavigationExpected.filter((record) => record.reachable === true);
  const currentMatrixGoverned = currentNavigationExpected.filter((record) => record.currentMatrixGoverned === true);
  const activeMissingGovernance = currentNavigationExpected.filter((record) =>
    record.capturePolicy !== "public-exception" && record.currentMatrixGoverned !== true,
  );
  ...
  const currentNavigationFailures = currentNavigationExpected.filter((record) => record.reachable === false);
  const inventoryProbeFailures = records.filter((record) =>
    record.currentNavigationExpected !== true && record.status === "navigation-failure",
  );
  const skippedUnsafe = records.filter((record) => record.reachable === null);
  const unclassified = records.filter((record) => record.lifecycle === "unclassified" || record.classification === "unclassified");
  const inTransit = records.filter((record) => record.lifecycle === "in-transit");
  const publicExceptions = records.filter((record) => record.capturePolicy === "public-exception");
  ...
  return {
    inventoriedSurfaces: records.length,
    ...
    standardWebmcpGovernedSurfaces: WEBMCP_STANDARD_SURFACE_TARGETS.length,
    webmcpGovernedSurfaces: currentMatrixGoverned.length,
    safelyReachableInventorySurfaces: safelyReachable.length,
    safelyReachableSurfaces: safelyReachable.length,
    currentNavigationSurfaces: currentNavigationExpected.length,
    currentNavigationReachableSurfaces: currentNavigationReachable.length,
    activeGovernedSurfaces: currentMatrixGoverned.length,
    activeMissingGovernance: activeMissingGovernance.map((record) => record.id),
    missingFromWebMcp: activeMissingGovernance.map((record) => record.id),
    inTransitSurfaces: inTransit.map((record) => record.id),
    ...
    currentNavigationFailures: currentNavigationFailures.map((record) => record.id),
    inventoryProbeFailures: inventoryProbeFailures.map((record) => record.id),
    navigationFailures: currentNavigationFailures.map((record) => record.id),
    ...
    inventoryReconciliationCoveragePct: percent(records.length - unclassified.length, records.length),
    reconciliationCoveragePct: percent(records.length - unclassified.length, records.length),
    currentNavigationReconciliationCoveragePct: percent(
      currentNavigationExpected.filter((record) => record.lifecycle !== "unclassified").length,
      currentNavigationExpected.length,
    ),
    governanceCoveragePct: percent(currentMatrixGoverned.length, currentNavigationExpected.length),
    coverageComplete: unclassified.length === 0 && activeMissingGovernance.length === 0,
  };
}
...
function markdownSummary(report) {
  const lines = [
    ...
    "",
    `Inventoried surfaces: ${report.summary.inventoriedSurfaces}`,
    `Standard WebMCP governed surfaces: ${report.summary.standardWebmcpGovernedSurfaces}`,
    `Current Matrix navigation surfaces: ${report.summary.currentNavigationSurfaces}`,
    `Current Matrix governed surfaces: ${report.summary.activeGovernedSurfaces}`,
    `Active missing governance: ${report.summary.activeMissingGovernance.length}`,
    `In-transit inventory-only surfaces: ${report.summary.inTransitSurfaces.length}`,
    `Current-navigation failures: ${report.summary.currentNavigationFailures.length}`,
    `Inventory probe failures: ${report.summary.inventoryProbeFailures.length}`,
    `Skipped/state-only: ${report.summary.skippedUnsafe.length}`,
    `Inventory reconciliation: ${report.summary.inventoryReconciliationCoveragePct}%`,
    `Current-navigation reconciliation: ${report.summary.currentNavigationReconciliationCoveragePct}%`,
    `Governance coverage: ${report.summary.governanceCoveragePct}%`,
    "",
  ];
  for (const [label, values] of [
    ["Active missing governance", report.summary.activeMissingGovernance],
    ["In-transit inventory-only", report.summary.inTransitSurfaces],
    ["Current-navigation failures", report.summary.currentNavigationFailures],
    ["Inventory probe failures", report.summary.inventoryProbeFailures],
...
export async function runWebMcpSurfaceCensusProfile({
  ...
  try {
    ...
    const connectedDashboardIds = new Set(
      dashboard.filter((entry) => entry.connected && !entry.disabled).map((entry) => entry.id),
    );

    for (const surface of SKIN_V1_SURFACES) {
      const webmcpGoverned = WEBMCP_STANDARD_SURFACE_TARGETS.includes(surface.id);
      const currentNavigationExpected = webmcpGoverned || connectedDashboardIds.has(surface.id);
      const currentMatrixGoverned = webmcpGoverned
        || (currentNavigationExpected && isCurrentMatrixSupplementalGoverned(surface.id));
      const probePage = await context.newPage();
      const probe = webmcpGoverned
        ? await probeStandardSurface(probePage, serverUrl, surface)
        : await probeCanonicalNonstandardSurface(probePage, serverUrl, surface);
      ...
      records.push({
        ...
        webmcpGoverned,
        currentNavigationExpected,
        currentMatrixGoverned,
        lifecycle: lifecycleForSurface({
          surfaceId: surface.id,
          capturePolicy: surface.capturePolicy,
          standardGoverned: webmcpGoverned,
          currentNavigationReachable: currentNavigationExpected,
        }),
        classification: currentMatrixGoverned
          ? "governed"
          : currentNavigationExpected
    ...
    for (const entry of dashboard) {
      ...
      const probePage = await context.newPage();
      const probe = await probeLiveExtra(probePage, serverUrl, entry);
      await probePage.close();
      const currentNavigationExpected = entry.connected && !entry.disabled;
      const currentMatrixGoverned = currentNavigationExpected && isCurrentMatrixSupplementalGoverned(entry.id);
      records.push({
        id: entry.id,
        label: entry.label,
        ...
        currentNavigationExpected,
        currentMatrixGoverned,
        lifecycle: currentNavigationExpected
          ? (currentMatrixGoverned ? "active-governed" : "unclassified")
          : "inventory-only",
        classification: currentNavigationExpected
          ? (currentMatrixGoverned ? "governed" : "unclassified")
          : "inventory-only",
        ...probe,
      });
    }
    ...
    const findings = [
      ...summary.activeMissingGovernance.map((surfaceId) => ({
        profile: "surface-census",
        severity: "diagnostic",
        category: "active-missing-governance",
        surfaceId,
      })),
      ...
      ...summary.currentNavigationFailures.map((surfaceId) => ({
        profile: "surface-census",
        severity: "diagnostic",
        category: "current-navigation-failure",
        surfaceId,
      })),
      ...summary.inventoryProbeFailures.map((surfaceId) => ({
        profile: "surface-census",
        severity: "diagnostic",
        category: "inventory-probe-failure",
        surfaceId,
      })),
    ];
    const blockerCount = summary.governedButUnreachable.length
      + summary.activeMissingGovernance.length
      + summary.currentNavigationFailures.length;
    const report = {
      schemaVersion: 1,
      profile: "surface-census",
      runId,
