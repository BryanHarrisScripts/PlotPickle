...
async function measureUnexpectedCls(browser, server, probe) {
  ...
  try {
    const response = await page.goto(new URL(`/ui-lab?probe=${encodeURIComponent(probe)}`, server).toString(), { waitUntil: "domcontentloaded", timeout: 30_000 });
    if (!response || response.status() >= 400) throw new Error(`UI experience lab ${probe} probe failed to render: HTTP ${response?.status() ?? "no response"}`);
...
async function inspectZoomAndLongContent(browser, server) {
  const context = await browser.newContext({ viewport: { width: 640, height: 720 } });
  const page = await context.newPage();
  try {
    const response = await page.goto(new URL("/ui-lab", server).toString(), { waitUntil: "domcontentloaded", timeout: 30_000 });
    if (!response || response.status() >= 400) throw new Error(`UI experience lab zoom probe failed to render: HTTP ${response?.status() ?? "no response"}`);
    await page.waitForSelector("[data-ui-experience-gallery='true']", { timeout: 10_000 });
...
async function inspectReducedMotion(browser, server) {
  const context = await browser.newContext({ viewport: { width: 1280, height: 720 }, reducedMotion: "reduce" });
  const page = await context.newPage();
  try {
    const response = await page.goto(new URL("/ui-lab", server).toString(), { waitUntil: "domcontentloaded", timeout: 30_000 });
    if (!response || response.status() >= 400) throw new Error(`UI experience lab reduced-motion probe failed to render: HTTP ${response?.status() ?? "no response"}`);
...
async function inspectStoryZeroRoute(browser, server) {
  const context = await browser.newContext({ viewport: { width: 640, height: 720 } });
  const page = await context.newPage();
  try {
    const response = await page.goto(new URL("/story", server).toString(), { waitUntil: "domcontentloaded", timeout: 30_000 });
...
export async function runUiExperienceAudit({ serverUrl, toolRoot } = {}) {
  ...
  try {
    for (const probe of UI_CLS_PROBES) {
      const cls = await measureUnexpectedCls(browser, server, probe);
    ...
    const zoom = await inspectZoomAndLongContent(browser, server);
    if (zoom.overflow > 1) failures.push(`UI lab overflowed horizontally by ${zoom.overflow}px at the 200% stress setting.`);
    ...
    const reducedMotionDurations = await inspectReducedMotion(browser, server);
    ...
    const story = await inspectStoryZeroRoute(browser, server);
