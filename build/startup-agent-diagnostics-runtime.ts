...
const SAGE_GROUNDING_CONCEPTS = [
  ["question", "proposition", "contested"],
  ["choice", "choices"],
...
const SAGE_DIAGNOSTIC_REPAIR_INSTRUCTION = [
  ...
  "In one to three sentences under 80 words, explain theme as a live question or contested proposition tested through choices and consequences.",
...
function groundingPass(answer: string) {
  ...
  return SAGE_GROUNDING_CONCEPTS.every((concept) => concept.some((term) => words.has(term)));
}
...
async function loadThemeProbe() {
  ...
  const lesson = lessons.find((item) => item.id === "essentials-theme")
    || lessons.find((item) => /theme/i.test(item.title || ""))
    || lessons[0];
  ...
  return {
    question: "According to this curriculum, what is theme, and how does a story test it? Answer in one to three sentences under 80 words.",
...
export async function runStartupAgentDiagnostics(baseUrl: string) {
  ...
  if (fastReady && sageRegistered) {
    try {
      ...
      printResult("Curriculum grounding", sage.grounded ? "PASS" : "FAIL", sage.grounded ? recoveryDetail : "expected theme + choices + consequences from curriculum context");
