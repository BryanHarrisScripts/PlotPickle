...
function toCanonicalLesson(entry: (typeof registry.lessons)[number]): CurriculumLesson {
  ...
  if (!source) throw new Error(`Canonical registry references missing authored lesson ${entry.id}.`);
  if (source.topic !== entry.topic) throw new Error(`Canonical topic drift for ${entry.id}.`);
  return {
    id: source.id,
    number: entry.number,
    topic: source.topic,
    ...
    apply: source.apply,
    tags: source.tags,
    original: { number: entry.number, path: `PlotPickle Enrichment / ${source.kind}` },
    sources: [],
  };
}
...
export function withIssue1976CanonicalEnrichment(documents: readonly TopicDocument[]): readonly TopicDocument[] {
  return documents.map((document) => {
    ...
    return { ...document, lessonCount: document.lessonCount + additions.length, lessons: [...document.lessons, ...additions] };
  });
}
