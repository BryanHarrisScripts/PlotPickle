...
function requireChromium(toolRoot) {
...
async function installCatalogueWebMcp(page, toolRoot) {
  ...
  await page.evaluate(async ({ names, registry, targets }) => {
    ...
    const contractForRenderedSurface = () => contracts.find((contract) => routeMatchesLocation(contract) && visible(document.querySelector(contract.readySelector)))
      || contracts.find((contract) => contract.id !== "dashboard" && visible(document.querySelector(contract.readySelector)))
      || (visible(document.querySelector(dashboard.readySelector)) ? dashboard : null);
...
async function capture(page, manifest, surface, failures) {
  ...
  if (surface === "dashboard") {
    ...
    try {
      ...
      failures.push("Dashboard is missing the visible PlotPickle Score mathematical panel; canonical visual evidence is incomplete.");
      return;
    }
  }
  const candidatePath = path.resolve(contract.candidate);
  await mkdir(path.dirname(candidatePath), { recursive: true });
  const candidate = surface === "writers-craft"
    ? await page.screenshot({ path: candidatePath, animations: "disabled", fullPage: false })
    : await locator.screenshot({ path: candidatePath, animations: "disabled" });
  ...
  const result = await comparePngPixels(page, baseline, candidate, manifest.comparison);
...
export async function runWebMcpStandardSurfaceCatalogue({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  const server = validateLocalServer(serverUrl || "http://127.0.0.1:4173");
  await waitForUiServer(server);
  const manifest = await loadManifest();
  const chromium = requireChromium(toolRoot);
