...
export async function runWebMcpSurfaceVisualAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    assert(settingsRows.map((item) => item.id).join(",") === "general,appearance,accessibility,defaults,advanced", "WebMCP Settings discovery did not match the final five-destination directory.", failures);
    assert(settingsRows.filter((item) => ["general", "appearance", "accessibility", "defaults"].includes(item.id)).every((item) => item.connected), "WebMCP Settings discovery did not expose all four active preference destinations.", failures);
    assert(settingsRows.some((item) => item.id === "advanced" && !item.connected), "WebMCP Settings discovery did not preserve Advanced as the explicit review destination.", failures);
  ...
  /*
  Historical candidate inventory retained for underlying surfaces no longer reachable from the primary Settings directory:
  inspectCurrent("cloud-story-mode"); captureSurfaceCandidate(page, manifest, "cloud-story-mode", failures);
  inspectCurrent("agents"); captureSurfaceCandidate(page, manifest, "agents", failures);
  inspectCurrent("local-ai"); captureSurfaceCandidate(page, manifest, "local-ai", failures);
  inspectCurrent("node"); captureSurfaceCandidate(page, manifest, "node", failures);
  */
