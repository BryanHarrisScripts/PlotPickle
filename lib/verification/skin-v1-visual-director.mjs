...
} from "./skin-v1/surface-contracts.mjs";
import {
...
export function analyzeVisualContinuity(reference, candidate, peerProfiles = [], contract = null) {
  ...
  const measurementPolicy = contract?.expected?.measurement || {};
  const spacingRhythmScope = measurementPolicy.spacingRhythmScope || "dashboard-four-pixel";
  const controlDensityProfile = measurementPolicy.controlDensityProfile || "dashboard-compact-controls";
  ...
  if (candidateRootHeight && heightLimit > 0 && candidateRootHeight > heightLimit + 16) {
    output.push(finding(
      candidate.surface,
      "advisory",
      "structural-height",
  ...
  if (spacingRhythmScope === "dashboard-four-pixel") {
    const offGrid = [];
    for (const item of candidateItems) {
    ...
    if (offGrid.length) {
      ...
      output.push(finding(candidate.surface, "advisory", "spacing-rhythm", first.item, first.property, first.value, "4px Dashboard/Skin V1 spacing grid", `${offGrid.length} padding value${offGrid.length === 1 ? " is" : "s are"} off the canonical four-pixel rhythm. Move governed standard-UI spacing onto Skin V1 spacing tokens to restore cross-screen cadence.`));
    }
  }

  const referencePanels = referenceItems.filter((item) => item.role === "panel");
  const flatPanels = candidateItems.filter((item) => item.role === "panel"
    && item.visualTreatment !== "flat-approved"
    && (!item.presentation?.backgroundImage || item.presentation.backgroundImage === "none"));
  ...
  if (controlDensityProfile === "dashboard-compact-controls") {
    const compactControls = (items) => items.filter((item) => item.role === "control" && item.controlDensityRole !== "content-card");
    const refControlMedian = median(compactControls(referenceItems).map((item) => px(item.presentation?.height)));
    const candidateControlMedian = median(compactControls(candidateItems).map((item) => px(item.presentation?.height)));
    if (refControlMedian && candidateControlMedian && candidateControlMedian > refControlMedian * 1.6) {
      output.push(finding(candidate.surface, "advisory", "control-density", null, "medianControlHeight", `${candidateControlMedian}px`, `near Dashboard's ${refControlMedian}px control rhythm`, "Standard UI controls are substantially taller than Dashboard's compact-control rhythm. Project workspaces use semantic control roles instead of this mixed median."));
    }
  }
  return output;
}
...
async function settleSurfacePresentation(page, rootSelector, maxWaitMs = 250) {
  await page.evaluate(async ({ selector, cap }) => {
    ...
    const nodes = [root, ...[...root.querySelectorAll("*")].slice(0, 1600)];
    let longest = 0;
    ...
    const waitMs = Math.min(cap, Math.max(0, Math.ceil(longest) + 16));
    if (waitMs > 0) await new Promise((resolve) => setTimeout(resolve, waitMs));
    await new Promise((resolve) => requestAnimationFrame(() => resolve()));
    await new Promise((resolve) => requestAnimationFrame(() => resolve()));
  }, { selector: rootSelector, cap: maxWaitMs });
}
...
async function collectProfile(page, surface, geometryEvidence) {
  ...
  await settleSurfacePresentation(page, contract.rootSelector);
...
export async function runSkinV1VisualDirector({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  const surfaceContractMatrix = await writeSkinV1SurfaceContractMatrix();
  ...
  try {
    ...
    for (const surface of SURFACES.slice(1)) {
      ...
      const findings = [
        ...analyzeVisualContinuity(dashboard, profiles[surface], peerProfiles, surfaceContract),
  ...
  console.log(`Surface Contract Matrix: ${surfaceContractMatrix.path}`);
  console.log(`Rendered geometry JSON + viewport screenshots: ${path.resolve(RENDERED_GEOMETRY_ARTIFACT_ROOT)}`);
