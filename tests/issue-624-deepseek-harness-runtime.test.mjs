...
test("launching DSH is an explicit user action through Ollama", async () => {
  ...
  assert.match(runtime, /spawn\("cmd\.exe", \["\/d", "\/c", "start", "", "cmd\.exe", "\/k", "ollama", "launch", "dsh"\]/);
  assert.doesNotMatch(runtime, /start \\"DeepSeek Harness\\" cmd \/k ollama launch dsh/);
  assert.doesNotMatch(runtime, /"\/s", "\/c"/);
