...
type LessonEnrichment = {
  ...
  readonly planOutput: readonly string[];
  ...
  "The Screenwriting Essentials Roadmap": {
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Before expanding into detailed structure, identify the three craft risks most likely to produce rework. This gives PLAN and the Creative Room a useful watchlist. The goal is not to eliminate uncertainty; it is to know where the story still needs proof.",
        ],
      },
    ],
    planOutput: [
  ...
  "Build the Story Experience": {
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "PLAN should treat the Foundations Brief as canonical but revisable. New scene evidence may reveal that the objective is wrong, the opposition is too weak or the theme is being stated rather than dramatized. Revise the Foundation deliberately, then let downstream structure and visuals inherit the approved change.",
        ],
      },
    ],
    planOutput: [
    ...
    sections: lesson.sections.map((section) => (
      section.heading === "Foundations table of contents"
        ? {
            ...section,
            paragraphs: [
              "Foundations is an eleven-lesson learning path. Read the lessons in order when you are new to the method, then revisit individual lessons when PLAN exposes a weak part of the story engine.",
            ],
            points,
          }
        : section
  ...
  const sequenced = FOUNDATION_SEQUENCE.map((title, index) => {
    ...
    return {
      ...
      sections: [
        ...
        ...lesson.sections,
        {
          heading: "Build this for PLAN",
          paragraphs: [
            "Do not try to make this perfect before moving on. Capture a defensible working answer, mark what remains uncertain, and let the next lessons test it. PLAN will keep this output beside the rest of the Foundation so the pieces can be revised together.",
          ],
          points: [...enrichment.planOutput],
        },
      ],
      exercise: enrichment.exercise ?? lesson.exercise,
      apply: "PLAN · Foundations",
