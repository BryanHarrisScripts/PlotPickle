...
export default function CommunityPresencePage() {
  ...
  function goPrevious(){if(window.history.length>1){window.history.back();return;}window.location.assign("/?workspace=community");}
  return <main style={{maxWidth:900,margin:"0 auto",padding:"34px 24px",color:"#eee9df"}}>
    <nav aria-label="Community Presence navigation" style={{display:"flex",gap:12,alignItems:"center",flexWrap:"wrap",marginBottom:20}}><button type="button" onClick={goPrevious}>Previous</button><Link href="/?workspace=dashboard">Home</Link><Link href="/?workspace=community">Community</Link></nav>
    <p><Link href="/studio-identity">Studio Identity</Link> / Community Presence</p><h1>Join the PlotPickle Community without exposing this Studio&apos;s local server.</h1>
