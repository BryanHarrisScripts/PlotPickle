...
export function validateAutonomousStoryRoutes(registry) {
  ...
  for (const route of routes) {
    ...
    if (route.expectedRoute !== undefined && (typeof route.expectedRoute !== "string" || !route.expectedRoute.startsWith("/") || route.expectedRoute.startsWith("//"))) pushUnique(errors, `${label} expectedRoute must be a local route.`);
...
export function assessAutonomousRoute(route, evidence = {}, action = {}) {
  ...
  const readinessVerified = evidence.readinessVerified === true;
  const matchedTerms = [];
  const missingTerms = [];
  for (const term of route.requiredTerms || []) {
    if (readinessVerified || normalized.includes(String(term).toLowerCase())) matchedTerms.push(term);
  ...
  if (!readinessVerified && bodyLength < Number(route.minimumTextLength || 0)) blockers.push(`${route.label} rendered too little visible content (${bodyLength} characters).`);
  ...
  const expectedRoute = route.expectedRoute || evidence.resolvedRoute;
  if (expectedRoute && actualRoute && actualRoute !== expectedRoute) {
  ...
  if (route.operation === "operate" && !action.attempted && !legitimateSkip && evidence.reached) {
    blockers.push(`${route.label} was entered but its declared autonomous operation did not execute.`);
  }
  if (route.operation === "operate" && action.attempted && action.succeeded && !String(action.operatorId || "").trim()) {
    blockers.push(`${route.label} operation did not identify the bounded autonomous operator.`);
  }
  ...
  return {
    ...
    readiness: {
      ...
      contractVerified: readinessVerified,
    },
    action: {
      ...
      canonicalProjectId: action.canonicalProjectId || "",
      decisionId: String(action.decisionId || "").slice(0, 180),
      revision: action.revision || "",
      writesCanon: action.writesCanon === true,
      receipt: { baseRevision: String(action.receipt?.baseRevision || "").slice(0, 120) },
      skippedPrerequisite,
    },
