...
const HUMAN_TEXT_REDACTIONS: ReadonlyArray<readonly [RegExp, string]> = [
  [/nsec1[a-z0-9]+/gi, "[private credential hidden]"],
  [RAW_HASH, "[technical reference hidden]"],
  [TRANSPORT_ID, "[technical reference hidden]"],
  [WINDOWS_PATH, "[local path hidden]"],
  [POSIX_PATH, "[local path hidden]"],
];

function cleanHumanText(value: string) {
  let cleaned = value;
  for (const [pattern, replacement] of HUMAN_TEXT_REDACTIONS) {
