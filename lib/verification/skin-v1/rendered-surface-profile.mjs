...
export async function collectRenderedSurfaceProfile(page, {
  ...
  return page.evaluate(({ surfaceName: name, rootSelector: selector, colorTokens: colors, contractTokens: tokens }) => {
    ...
    const typographyRole = (node) => {
      if (node.matches(".pp-skin-v1-dashboard-brand h1,[data-skin-typography='display']")) return "display";
      if (node.matches("h1")) return "h1";
      if (node.matches("h2")) return "h2";
      if (node.matches("h3")) return "h3";
      if (node.matches("h4")) return "h4";
      if (node.matches("h5")) return "h5";
      if (node.matches("h6")) return "h6";
      if (node.matches("[data-keycap],[data-dashboard-shortcut],[data-settings-shortcut],[data-library-shortcut]")) return "keycap";
      if (node.matches("[role='status'],[role='alert'],[aria-live],.pp-skin-v1-status,footer")) return "status";
      if (node.matches("[role='menuitem'],[role='tab'],.pp-skin-v1-menu-item,[data-dashboard-menu-item],[data-settings-secondary-item],[data-library-nav]")) return "menu";
      if (node.matches("button,input:not([type='hidden']),select,textarea,[role='button'],[role='option']")) return "control";
      if (node.matches("small,[data-skin-typography='note']")) return "note";
      if (node.matches("label,dt,dd,[data-skin-typography='detail']")) return "detail";
      if (node.matches("p,li")) return "body";
      return node.getAttribute("data-skin-typography") || null;
    };
    ...
    return {
      ...
      items: nodes.map((node, index) => ({
        role: role(node),
        typographyRole: typographyRole(node),
        identity: identity(node, index),
        presentation: presentation(node),
      })),
    };
...
export function analyzeRenderedSurfaceContract(contract, profile) {
  ...
  const matchingGrid = Number.isInteger(expectedColumns)
    ? gridCandidates
      ...
      .sort((left, right) => (right.rect.width * right.rect.height) - (left.rect.width * left.rect.height))[0] || null
    : null;
  const primaryGrid = matchingGrid || geometry.primaryGrid;
  const primaryGridIsShellEvidence = primaryGrid
    && Number(primaryGrid.rootWidthRatio) >= 0.65
    && Number(primaryGrid.rootHeightRatio) >= 0.2;
  if (Number.isInteger(expectedColumns) && primaryGridIsShellEvidence && primaryGrid.columns !== expectedColumns) {
    findings.push(finding(
      surface,
      "blocker",
      "column-contract",
  ...
  for (const item of profile.items || []) {
    ...
    if (!roleName || !expectedRole) continue;
