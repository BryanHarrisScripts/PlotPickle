...
async function goDashboard(page, server) {
  ...
  if (new URL(page.url()).pathname !== "/skin-v1") {
    ...
    if (!response || response.status() >= 400) throw new Error(`Visual Director could not restore the Dashboard route: HTTP ${response?.status() ?? "no response"}.`);
  ...
  for (let depth = 0; depth < 8; depth += 1) {
    if (await page.locator(dashboard.readySelector).first().isVisible().catch(() => false)) return;
    const orchestratedDirect = page.locator("button.pp-skin-v1-orchestrator-return:visible").filter({ hasText: "Dashboard" }).first();
    if (await orchestratedDirect.count()) {
      await orchestratedDirect.click();
    } else {
      const orchestratedBack = page.locator("button.pp-skin-v1-orchestrator-return:visible").first();
      if (await orchestratedBack.count()) {
        await orchestratedBack.click();
      } else {
...
async function collectProfile(page, surface) {
  ...
  return page.evaluate(({ surfaceName, rootSelector, colorTokens, contractTokens }) => {
    ...
    const identity = (node, index) => node.getAttribute("data-skin-id") || node.getAttribute("data-dashboard-menu-item") || node.getAttribute("data-settings-secondary-item") || node.getAttribute("data-library-nav") || node.id || node.getAttribute("aria-label") || node.getAttribute("alt") || node.getAttribute("name") || `${node.tagName.toLowerCase()}#${index}`;
...
export async function runSkinV1VisualDirector({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    for (const surface of SURFACES.slice(1)) profiles[surface] = await navigateAndCollect(page, surface, server);
