  ...
  for (const row of result.rows) {
    ...
    if (!result.indicatorsHidden) {
      ...
      const isDashboardReviewItem = menuName === "dashboard"
        && ["library", "plan", "storyboard", "previs"].includes(row.id);
...
export async function runSkinV1MenuContractAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    await clickSurfaceReturn(page, "Back to Library");
    await page.locator("section[aria-label='Library menu'][data-library-directory='keyboard-directory']").waitFor({ state: "visible", timeout: 15_000 });
    if (await page.locator("[data-library-destination]").count()) {
      addFailure(failures, violation("LIBRARY", "library-nested-return", "nestedDestination", "still-open", "closed"));
    }
    ...
    if (settingsIds !== "general,story-mode,node-info,agents") {
      addFailure(failures, violation("SETTINGS", "settings-directory", "destinations", settingsIds, "general,story-mode,node-info,agents"));
    }

    const generalRow = page.locator("[data-settings-secondary-item='general']");
    await generalRow.focus();
    await page.keyboard.press("ArrowDown");
    await expectSelected(page, "[data-settings-secondary-item='story-mode']", "SETTINGS", "settings-story-mode", failures);
    ...
    await page.locator("[data-settings-secondary-item='story-mode']").focus();
    await page.keyboard.press("S");
    await page.locator("section[aria-label='Story Mode directory'][data-story-mode-parent='true']").waitFor({ state: "visible", timeout: 20_000 });

    const localStoryModeRow = page.locator("[data-story-mode-policy='local']");
    await localStoryModeRow.focus();
    await page.keyboard.press("L");
    ...
    await clickSurfaceReturn(page, "Back to Story Mode");
    await page.locator("section[aria-label='Story Mode directory'][data-story-mode-parent='true']").waitFor({ state: "visible", timeout: 15_000 });

    const cloudStoryModeRow = page.locator("[data-story-mode-policy='cloud']");
    await cloudStoryModeRow.focus();
    await page.keyboard.press("C");
    ...
    await page.locator("[data-settings-secondary-item='general']").focus();
