...
export function resolveSkinV1GovernedSurfaceComposition(surface, grammar) {
  ...
  const inheritedPrimitives = unique(familyProfile.primitives || []);
  const requiredPrimitives = unique(declared.requires || []);
  const excludedPrimitives = new Set(declared.excludes || []);
  const effectivePrimitives = unique([...inheritedPrimitives, ...requiredPrimitives])
...
export function resolveSkinV1BlastRadius(sources, change) {
  ...
  if (change?.type === "primitive") {
    ...
    affectedFamilies = unique(matches.map((composition) => composition.familyProfile));
  ...
  return {
    ...
    affectedFamilies: unique(affectedFamilies).sort(),
    affectedSurfaces: unique(affectedSurfaces).sort(),
    affectedOverlays: unique(affectedOverlays).sort(),
  };
}
