...
function governedPrimitive(grammar, id) {
  const primitive = grammar.governedPrimitives?.[id];
  if (!primitive) throw new Error(`Unknown governed primitive ${id}.`);
  return primitive;
}

export function resolveSkinV1GovernedSurfaceComposition(surface, grammar) {
  const declared = surface.primitiveContract;
  if (!declared) return null;
  const familyProfileId = declared.familyProfile || surface.family;
  const familyProfile = grammar.surfaceFamilyProfiles?.[familyProfileId];
  if (!familyProfile) throw new Error(`Missing governed family profile ${familyProfileId} for ${surface.id}.`);

  const inheritedPrimitives = [...new Set(familyProfile.primitives || [])];
  const requiredPrimitives = [...new Set(declared.requires || [])];
  const excludedPrimitives = new Set(declared.excludes || []);
  const effectivePrimitives = [...new Set([...inheritedPrimitives, ...requiredPrimitives])]
    .filter((id) => !excludedPrimitives.has(id));

  for (const id of effectivePrimitives) governedPrimitive(grammar, id);
  ...
  return {
    ...
    inheritedPrimitives,
    requiredPrimitives,
    excludedPrimitives: [...excludedPrimitives],
    effectivePrimitives,
  };
}

export function resolveSkinV1GovernedCompositions(sources) {
  return sources.registry.surfaces
    .map((surface) => resolveSkinV1GovernedSurfaceComposition(surface, sources.grammar))
    .filter(Boolean);
}

export function resolveSkinV1BlastRadius(sources, change) {
  const compositions = resolveSkinV1GovernedCompositions(sources);
  const registryById = new Map(sources.registry.surfaces.map((surface) => [surface.id, surface]));
  const overlayProfiles = sources.grammar.overlayProfiles || {};
  let affectedSurfaces = [];
  let affectedFamilies = [];
  let affectedOverlays = [];

  if (change?.type === "primitive") {
    governedPrimitive(sources.grammar, change.id);
    const matches = compositions.filter((composition) => composition.effectivePrimitives.includes(change.id));
    affectedSurfaces = matches.map((composition) => composition.surfaceId);
    affectedFamilies = [...new Set(matches.map((composition) => composition.familyProfile))];
    affectedOverlays = Object.entries(overlayProfiles)
      .filter(([, profile]) => (profile.primitives || []).includes(change.id))
      .map(([id]) => id);
  } else if (change?.type === "family") {
    if (!sources.grammar.surfaceFamilyProfiles?.[change.id]) {
      throw new Error(`Unknown governed surface family ${change.id}.`);
    }
    affectedFamilies = [change.id];
    affectedSurfaces = compositions
      .filter((composition) => composition.familyProfile === change.id)
      .map((composition) => composition.surfaceId);
  } else if (change?.type === "surface") {
    const surface = registryById.get(change.id);
    if (!surface) throw new Error(`Unknown governed surface ${change.id}.`);
    affectedSurfaces = [...new Set([
      surface.id,
      ...(surface.blastRadiusDependents || []).filter((id) => registryById.has(id)),
    ])];
    const composition = resolveSkinV1GovernedSurfaceComposition(surface, sources.grammar);
    affectedFamilies = composition ? [composition.familyProfile] : [];
  } else {
    throw new Error("Skin V1 blast-radius change must be primitive, family, or surface.");
  }

  return {
    change: { type: change.type, id: change.id },
    affectedFamilies: [...new Set(affectedFamilies)].sort(),
    affectedSurfaces: [...new Set(affectedSurfaces)].sort(),
    affectedOverlays: [...new Set(affectedOverlays)].sort(),
  };
}
...
export function resolveSkinV1SurfaceContracts(sources) {
  ...
  return standard.map((surface) => {
    ...
    return {
      ...
      formatProfile,
      governedComposition: resolveSkinV1GovernedSurfaceComposition(surface, grammar),
