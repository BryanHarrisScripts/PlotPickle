...
export type VisualContextSource = {
  kind: VisualContextSourceKind;
  id: string;
  label: string;
};

export type VisualStoryContextTarget = {
...
function source(kind: VisualContextSourceKind, id: string, label: string): VisualContextSource {
  return { kind, id, label };
}

function target(kind: ConceptCanvasTargetKind, id: string, label: string): VisualStoryContextTarget {
  return { kind, id: id || "project", label: label || "Whole project" };
}
...
export function assembleVisualStoryContext(project: PlotPickleProject, requestedTarget?: Partial<VisualStoryContextTarget>): VisualStoryContextPackage {
  ...
  const contextTarget = target(
    requestedTarget?.kind ?? canvas.targetKind,
    requestedTarget?.id ?? canvas.targetId,
    requestedTarget?.label ?? canvas.targetLabel,
  );
  ...
  return {
    ...
    project: {
      ...
      visualVision: project.development.pitch.visualVision,
    },
    story: {
      premise: project.story.premise,
      logline: project.story.logline,
      theme: project.story.theme,
      dramaticQuestion: project.story.dramaticQuestion,
      stakes: project.story.stakes,
      ending: project.story.ending,
    },
    world: {
      ordinaryWorld: project.world.ordinaryWorld,
      newWorld: project.world.newWorld,
    ...
    concept: {
      text: canvas.conceptText,
      emotionalPurpose: canvas.emotionalPurpose,
      audienceExperience: canvas.audienceExperience,
      desiredVisualImpact: canvas.desiredVisualImpact,
      mustKeepConstraints: canvas.mustKeepConstraints,
      openExploration: canvas.openExploration,
    },
