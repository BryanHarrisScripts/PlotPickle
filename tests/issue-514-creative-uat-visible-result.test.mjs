...
test("Creative Writer UAT keeps PASS results visible until Enter", async () => {
  ...
  assert.match(script, /AGENT COMPLETED/);
  assert.match(script, /Result: Creative Writer acceptance report produced successfully/);
...
test("Creative Writer UAT keeps failures visible instead of throwing out of the launched window", async () => {
  ...
  assert.match(script, /AGENT NEEDS ATTENTION/);
  assert.match(script, /Result: \$Message/);
