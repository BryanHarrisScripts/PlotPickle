...
if (ids.join(",") !== "1,2,3,4,5,6,7") {
  throw new Error(`WebMCP QA profile registry must define exactly profiles 1 through 7; found ${ids.join(",") || "none"}.`);
}
...
export const WEBMCP_FULL_QA_ORDER = Object.freeze(["1", "2", "3", "4", "5", "6"]);
...
export function resolveWebMcpQaProfile(value = WEBMCP_QA_PROFILE_CONFIG.defaultProfile) {
  ...
  if (!match) throw new Error(`Unknown WebMCP QA profile: ${value || "empty"}. Choose 1 through 7.`);
  return match;
}
