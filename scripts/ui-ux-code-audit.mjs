...
function suggestionAlreadySatisfied(finding) {
  ...
  const requestedAttributes = [...finding.suggestion.matchAll(/\b(?:add|include|apply|set)\s+(?:a\s+|an\s+)?(?:meaningful\s+|descriptive\s+)?[`'\"]?(alt|aria-[\w-]+|role|tabindex|loading)[`'\"]?(?:\s+attribute)?(?:\s*=\s*[`'\"]?([\w-]+))?/gi)];
...
function referencedIdsExist(finding, source) {
  const claimsMissing = /(?:does not exist|missing|invalid reference|cannot be found|ensure[^.]*exists)/i.test(`${finding.message} ${finding.suggestion}`);
  if (!claimsMissing) return false;

  const literalReference = finding.evidence.match(/aria-(?:labelledby|describedby)=["']([^"']+)["']/i);
  if (literalReference) {
    return literalReference[1]
      .split(/\s+/)
      .filter(Boolean)
      .every((id) => new RegExp(`id=["']${escapeRegExp(id)}["']`).test(source));
  }

  const expressionReference = finding.evidence.match(/aria-(?:labelledby|describedby)=\{([A-Za-z_$][\w$]*)\}/i);
  if (expressionReference) {
    return new RegExp(`id=\\{${escapeRegExp(expressionReference[1])}\\}`).test(source);
  }
...
function ariaClaimIsUnsubstantiated(finding) {
  if (finding.criterion !== 15) return false;
  const combined = `${finding.message} ${finding.suggestion}`;
  if (!/(?:invalid\s+aria|aria[^.]*invalid|not\s+valid[^.]*aria)/i.test(combined)) return false;
...
function evidenceIsInsideNavigationLandmark(finding, source) {
  ...
  const evidenceIndex = source.indexOf(finding.evidence);
  if (evidenceIndex < 0) return false;
  const before = source.slice(0, evidenceIndex);
  const after = source.slice(evidenceIndex + finding.evidence.length);
  return before.lastIndexOf("<nav") > before.lastIndexOf("</nav>") && after.includes("</nav>");
}
...
function contradictsPlatformStandards(finding, source, context) {
  ...
  if (finding.criterion === 11 && /<h[1-6]\b/i.test(evidence) && /(?:non-semantic|not semantic|use a semantic heading|replace[^.]*semantic heading)/i.test(combined)) return true;
  ...
  if (finding.criterion === 15 && /<(?:dd|span|p|div)\b/i.test(evidence) && /(?:aria-live|role=["']?status|live region|announce(?:d|ment)?)/i.test(combined) && !/\b(?:error|alert|save|saving|saved|progress|upload|download|retry|success|failure|failed)\b/i.test(evidence)) return true;
  if (finding.criterion === 11 && /<div\b/i.test(evidence) && /(?:replace[^.]*button|add\s+role=["']?button|make it accessible)/i.test(combined) && !/\b(?:onClick|onKeyDown|onKeyUp|onPointerDown|role=["']button["']|tabIndex)\b/i.test(evidence)) return true;
  ...
  if (finding.criterion === 15 && /<nav\b[^>]*\baria-(?:label|labelledby)=/i.test(evidence) && /(?:invalid\s+aria|aria[^.]*invalid|not\s+valid[^.]*aria)/i.test(combined)) return true;
