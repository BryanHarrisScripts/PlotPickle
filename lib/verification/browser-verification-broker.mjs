...
class BrowserDiagnosticsObserver {
  ...
  attachPage(page) {
    ...
    page.on("console", (message) => {
      ...
      const resourceConsoleError = type === "error" && /^Failed to load resource:/u.test(detail);
      this.record(page, {
        category: resourceConsoleError ? "console-resource-error" : (type === "error" ? "console-error" : "console-warning"),
        severity: resourceConsoleError ? "advisory" : (type === "error" ? "blocker" : "advisory"),
        summary: resourceConsoleError ? "Browser reported a failed resource request" : `console.${type}`,
        detail,
      });
    });
    ...
    page.on("response", (response) => {
      ...
      this.record(page, {
        ...
        severity: status >= 500 || required ? "blocker" : "advisory",
  ...
  return {
    ...
    async close() {
      ...
      if (failOnBlockers && summary.totals.blocker > 0) {
        const blockers = summary.findings
          .filter((finding) => finding.severity === "blocker" && !finding.allowlistMatch)
          .slice(0, 5)
          .map((finding) => `${finding.category}: ${finding.summary}${finding.sanitizedDetail ? ` — ${finding.sanitizedDetail}` : ""}`)
          .join("; ");
        const error = new Error(`Browser diagnostics found ${summary.totals.blocker} blocking browser-health finding(s): ${blockers}. Evidence: ${path.posix.join(artifactRoot.replaceAll("\\", "/"), runId, "browser-diagnostics.json")}`);
