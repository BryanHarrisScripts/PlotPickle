...
The consolidated `PlotPickle Visual Gate` runs for every pull request targeting `main` so GitHub always receives the required check named `Visual`.
...
- focused workspace and accessibility contracts inside the Visual gate;
...
3. Keep `Visual` in `config/public-repository.settings.json` so the stable status context is always present on `main` pull requests.
