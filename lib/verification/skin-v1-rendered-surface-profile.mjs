...
export function analyzeRenderedSurfaceContract(contract, profile) {
  ...
  if (expectedLabels.length) {
    const renderedLabels = new Set((geometry.menus || []).flatMap((menu) => menu.items || []).map((item) => normalizedLabel(item.label)));
    const missing = expectedLabels.filter((label) => !renderedLabels.has(normalizedLabel(label)));
    if (missing.length) {
      findings.push(finding(
        surface,
        "advisory",
