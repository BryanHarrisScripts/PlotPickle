...
export async function observeRenderedUi(client, textExtractor = defaultResultText) {
  const result = await client.call("browser_evaluate", { function: `() => {
    ...
    for (const node of [...document.querySelectorAll('main *, body > *')]) {
      ...
      lightSurfaces.push({ tag: node.tagName.toLowerCase(), label: (node.getAttribute('aria-label') || node.textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 90), background: style.backgroundColor, rect: rect(node) });
    ...
    const clippedControls = controls.map((node) => ({ label: (node.getAttribute('aria-label') || node.textContent || node.getAttribute('placeholder') || '').replace(/\\s+/g, ' ').trim().slice(0, 90), rect: rect(node) }))
    ...
    for (let i = 0; i < controls.length && overlaps.length < 8; i += 1) {
      ...
      for (let j = i + 1; j < controls.length && overlaps.length < 8; j += 1) {
        ...
        if (width > 4 && height > 4) overlaps.push({ first: (controls[i].getAttribute('aria-label') || controls[i].textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 70), second: (controls[j].getAttribute('aria-label') || controls[j].textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 70), width: Math.round(width), height: Math.round(height) });
      }
    }
    const majorRegions = [...document.querySelectorAll('main > section, main > article, main > aside, main > nav, main > details')].filter(visible).slice(0, 12).map((node) => ({ tag: node.tagName.toLowerCase(), label: (node.getAttribute('aria-label') || node.querySelector('h1,h2,h3')?.textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 80), rect: rect(node) }));
