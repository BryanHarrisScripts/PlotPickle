...
export async function publishDailyRadar({
  ...
  let ossRules = { schemaVersion: 1, status: "unavailable", reason: "OSS Rules enrichment was not configured.", repositoriesChecked: 0, findings: [], checks: [] };
  if (ossRulesResearch) {
    try {
      ossRules = await ossRulesResearch({ candidates: selection.reviewQueue, fetchImpl });
    } catch (error) {
      ossRules.reason = String(error?.message || error).slice(0, 160);
    }
  }
  for (const candidate of selection.reviewQueue) {
    candidate.ossRulesSignal = ossRules.findings?.find((item) => item.repository === candidate.fullName) || null;
  }
  const rendered = renderDailyReport({ reportDate, selection, contract, history, ossRules });
