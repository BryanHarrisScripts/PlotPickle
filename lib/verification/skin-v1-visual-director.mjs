...
async function collectProfile(page, surface) {
  ...
  return page.evaluate(({ surfaceName, rootSelector, colorTokens, contractTokens }) => {
    ...
    const identity = (node, index) => node.getAttribute("data-skin-id") || node.getAttribute("data-dashboard-menu-item") || node.getAttribute("data-settings-secondary-item") || node.getAttribute("data-library-nav") || node.id || node.getAttribute("aria-label") || node.getAttribute("name") || `${node.tagName.toLowerCase()}#${index}`;
...
export async function runSkinV1VisualDirector({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    const dashboard = await navigateAndCollect(page, "dashboard", server);
    for (const surface of SURFACES.slice(1)) {
      const findings = analyzeVisualContinuity(dashboard, await navigateAndCollect(page, surface, server));
      surfaces[surface] = {
