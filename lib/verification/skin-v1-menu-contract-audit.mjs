...
export async function runSkinV1MenuContractAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    const learnMenu = page.locator("section[aria-label='LEARN menu'][data-skin-menu='learn-journey']");
    await learnMenu.waitFor({ state: "visible", timeout: 20_000 });
    await inspectMenu(page, "learn-journey", failures);
    const learnPathCount = await page.locator("[data-learn-semester-open='true']").count();
    if (learnPathCount !== 6) {
      addFailure(failures, violation("LEARN", "direct-paths", "pathCount", learnPathCount, 6));
    }
    const exploreEntry = page.locator("[data-learn-explore-open='true']");
    await exploreEntry.focus();
    ...
    await allCurriculum.getByRole("button", { name: "Back to Learn", exact: true }).click();
    await learnMenu.waitFor({ state: "visible", timeout: 20_000 });
    await clickSurfaceReturn(page, "Back to Dashboard");
