...
const DASHBOARD_REVIEW_IDS = new Set([
  "screening",
  "sound-narration",
  "sound-music",
  "sound-foley",
  ...
  for (const row of result.rows) {
    ...
    if (!result.indicatorsHidden) {
      ...
      const isDashboardReviewItem = menuName === "dashboard"
        && DASHBOARD_REVIEW_IDS.has(row.id);
...
export function summarizeDashboardMaturity(rows = []) {
  const normalized = rows.map((row) => ({
    id: String(row.id || ""),
    state: ["locked", "in-review", "unavailable"].includes(row.surfaceState) ? row.surfaceState : "unavailable",
    connected: Boolean(row.connected),
    locked: Boolean(row.locked),
  }));
  return {
    rows: normalized,
    locked: normalized.filter((row) => row.state === "locked").map((row) => row.id),
    inReview: normalized.filter((row) => row.state === "in-review").map((row) => row.id),
    unavailable: normalized.filter((row) => row.state === "unavailable").map((row) => row.id),
  };
}

async function readDashboardMaturity(page) {
  const rows = await page.locator("[data-dashboard-menu-item]").evaluateAll((nodes) => nodes.map((row) => ({
    id: row.getAttribute("data-dashboard-menu-item") || "",
    connected: row.getAttribute("data-dashboard-connected") === "true",
    surfaceState: row.getAttribute("data-dashboard-surface-state") || "",
    locked: row.getAttribute("data-dashboard-locked") === "true",
  })));
  return summarizeDashboardMaturity(rows);
}

async function verifyDashboardKeyboardTraversal(page, failures) {
  const ids = await page.locator("[data-dashboard-menu-item]").evaluateAll((nodes) => nodes.map((node) => node.getAttribute("data-dashboard-menu-item") || ""));
  if (!ids.length) {
    addFailure(failures, violation("DASHBOARD", "dashboard-keyboard-order", "menuRows", 0, ">= 1"));
    return;
  }

  const row = (id) => page.locator(`[data-dashboard-menu-item='${id}']`);
  await row(ids[0]).focus();
  for (let index = 1; index < ids.length; index += 1) {
    await page.keyboard.press("ArrowDown");
    await expectSelected(page, `[data-dashboard-menu-item='${ids[index]}']`, "DASHBOARD", `dashboard-${ids[index]}`, failures);
  }
  await page.keyboard.press("ArrowDown");
  await expectSelected(page, `[data-dashboard-menu-item='${ids[0]}']`, "DASHBOARD", "dashboard-wrap-first", failures);

  await page.keyboard.press("ArrowUp");
  await expectSelected(page, `[data-dashboard-menu-item='${ids.at(-1)}']`, "DASHBOARD", "dashboard-wrap-last", failures);

  await page.keyboard.press("Home");
  await expectSelected(page, `[data-dashboard-menu-item='${ids[0]}']`, "DASHBOARD", "dashboard-home", failures);

  await page.keyboard.press("End");
  await expectSelected(page, `[data-dashboard-menu-item='${ids.at(-1)}']`, "DASHBOARD", "dashboard-end", failures);
}
...
export async function runSkinV1MenuContractAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  const failures = [];
  let dashboardMaturity = { rows: [], locked: [], inReview: [], unavailable: [] };
  ...
  try {
    ...
    dashboardMaturity = await readDashboardMaturity(page);
    await verifyDashboardKeyboardTraversal(page, failures);
    ...
    await resetCommunityRow.focus();
    await page.keyboard.press("ArrowDown");
    await expectSelected(page, "[data-dashboard-menu-item='screening']", "DASHBOARD", "dashboard-screening", failures);
    await page.keyboard.press("ArrowDown");
    await expectSelected(page, "[data-dashboard-menu-item='reports']", "DASHBOARD", "dashboard-reports", failures);
    await page.keyboard.press("ArrowDown");
  ...
  return { menus: ["dashboard", "settings"], status: "pass", dashboardMaturity };
}