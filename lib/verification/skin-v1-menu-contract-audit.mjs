...
export async function runSkinV1MenuContractAudit({ serverUrl, toolRoot, storageStatePath = "" } = {}) {
  ...
  try {
    ...
    if (settingsIds !== "general,appearance,accessibility,defaults,advanced") {
      addFailure(failures, violation("SETTINGS", "settings-directory", "destinations", settingsIds, "general,appearance,accessibility,defaults,advanced"));
    }
    ...
    await generalRow.focus();
    await page.keyboard.press("C");
    await page.locator("section[aria-label='accessibility settings']").waitFor({ state: "visible", timeout: 15_000 });
    await inspectSolidChrome(page, "ACCESSIBILITY", failures);
    await page.getByRole("button", { name: "Back to Settings" }).click();
    await page.locator("section[aria-label='Settings menu'][data-skin-menu='settings']").waitFor({ state: "visible", timeout: 15_000 });

    const defaultsSourceRow = page.locator("[data-settings-secondary-item='general']");
    await defaultsSourceRow.focus();
    await page.keyboard.press("D");
    await page.locator("section[aria-label='defaults settings']").waitFor({ state: "visible", timeout: 15_000 });
    await inspectSolidChrome(page, "DEFAULTS", failures);
    await page.getByRole("button", { name: "Back to Settings" }).click();
    await page.locator("section[aria-label='Settings menu'][data-skin-menu='settings']").waitFor({ state: "visible", timeout: 15_000 });

    const advancedSourceRow = page.locator("[data-settings-secondary-item='general']");
    await advancedSourceRow.focus();
    await page.keyboard.press("V");
    await expectSelected(page, "[data-settings-secondary-item='advanced']", "SETTINGS", "settings-advanced", failures);
    await page.locator("[data-settings-secondary-item='advanced']").press("Enter");
    await page.locator("section[aria-label='advanced settings review']").waitFor({ state: "visible", timeout: 15_000 });
    await inspectSolidChrome(page, "ADVANCED", failures);
    await page.getByRole("button", { name: "Back to Settings" }).click();
    await page.locator("section[aria-label='Settings menu'][data-skin-menu='settings']").waitFor({ state: "visible", timeout: 15_000 });
    ...
    const allCurriculum = page.locator("section[aria-label='LEARN Explore All Curriculum'][data-learn-explore-access='unrestricted']");
    ...
    if (exploreLessonCount < 1) {
      addFailure(failures, violation("LEARN_EXPLORE", "all-curriculum-results", "visibleLessonCount", exploreLessonCount, ">= 1"));
    }
  ...
  /*
  Historical Story Mode conformance evidence remains source-owned after #2044. These components are no longer
  primary Settings destinations, but their contracts remain tested independently:
  CLOUD_STORY_MODE / LOCAL_AI / [data-skin-chrome='solid']
  keyboard.press("L")
  inspectMenu(page, "cloud-story-mode", failures);
  inspectMenu(page, "local-story-mode", failures);
  data-cloud-story-view='openai'
  data-local-ai-view='ollama'
  Previous live return list: ["dashboard", "settings", "cloud-story-mode", "profile", "local-story-mode"].
  Historical #1793 shortcut evidence: keyboard.press("N").
  */
