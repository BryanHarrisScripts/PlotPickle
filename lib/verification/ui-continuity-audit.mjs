...
function shellExpected(screen) {
  return screen.kind === "workspace"
    || screen.kind === "nested-workspace"
    || screen.migrationClass === "canonical"
    || screen.migrationClass === "contextual";
}

function forgivingShellPresent(snapshot) {
  return Boolean(clean(snapshot?.activeArea))
    && Number(snapshot?.navigationAreaCount || 0) > 0
    && Number(snapshot?.canonicalDestinationCount || 0) > 0;
}
...
export function auditContinuitySnapshot(screen, snapshot, baseline = null) {
  const findings = [];
  const expectsShell = shellExpected(screen);
  const hasForgivingShell = forgivingShellPresent(snapshot);
  ...
  if (expectsShell) {
    if (snapshot?.shell?.contract !== "v1" && !hasForgivingShell) findings.push(issue("shared-shell", "The canonical shared application shell is missing.", "error"));
    if (snapshot?.shell?.designSystem !== "matte-black-teal-orange" && !hasForgivingShell) findings.push(issue("design-system", "The shared shell is not marked with the approved matte-black/teal-orange design contract."));
    ...
    if (screen.expectedArea && clean(snapshot?.activeArea) !== clean(screen.expectedArea)) {
      findings.push(issue(
        "sitemap-area",
        `The migrated screen reports area ${clean(snapshot?.activeArea) || "none"}; expected ${clean(screen.expectedArea)}.`,
        "error",
      ));
    }
    if (screen.expectedDestination && clean(snapshot?.activeDestination) !== clean(screen.expectedDestination)) {
      findings.push(issue(
        "sitemap-destination",
        `The migrated screen reports destination ${clean(snapshot?.activeDestination) || "none"}; expected ${clean(screen.expectedDestination)}.`,
        "error",
      ));
    }
  ...
  } else if (screen.migrationClass === "public-exception" && clean(snapshot?.activeArea)) {
    findings.push(issue(
      "public-shell-boundary",
      `Public/Startup exception ${screen.label} was unexpectedly wrapped in creative area ${clean(snapshot.activeArea)}.`,
      "error",
    ));
  }
...
export function continuityReport({ generatedAt, server, results, runtimeError = "" }) {
  ...
  for (const result of results) {
    ...
    if (result.passed) lines.push("Shared-shell orientation, sitemap migration context, canonical reachability, project/status context, theme, anchor and return-control contracts passed.", "");
