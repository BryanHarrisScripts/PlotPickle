...
async function readMessages(channelId: string): Promise<BuzzMessage[]> {
  if (!channelId) return [];
  const response = await fetch(`${BUZZ_API}/messages?channel=${encodeURIComponent(channelId)}&limit=24`, {
    cache: "no-store",
...
export default function CommunityBackdoorTerminal({
  ...
  const talkTargets = useMemo<TalkTarget[]>(() => {
    const agents = BUZZ_GUILDHALL_ACTORS.flatMap<TalkTarget>((actor) => {
      ...
      return [{
        ...
        detail: `${actor.title} · ${room.label}`,
        channelId: room.channelId,
        routeLabel: `SIGNED BUZZ HOME ROOM · ${room.label}`,
      }];
    });
    const people = greatHallChannelId ? members.map<TalkTarget>((member) => ({
      key: `member:${member.pubkey}`,
      kind: "member",
      id: member.pubkey,
      label: member.displayName,
      detail: `${member.presence || "offline"} · Great Hall member`,
      channelId: greatHallChannelId,
  ...
  const selectedTarget = useMemo(
    () => talkTargets.find((target) => target.key === selectedTargetKey) ?? null,
    [selectedTargetKey, talkTargets],
  );
  ...
  const loadTalkMessages = useCallback(async (target: TalkTarget | null) => {
    ...
    try {
      const messages = await readMessages(target.channelId);
  ...
  useEffect(() => {
    void loadTalkMessages(selectedTarget);
  }, [loadTalkMessages, selectedTarget]);
  ...
  return (
    <main className={styles.frame} data-community-terminal="backdoor-v1" data-halted={halted ? "true" : "false"}>
      <section className={styles.crt} aria-label="PlotPickle Community backdoor terminal">
        ...
        <div className={styles.terminalGrid}>
          <div className={styles.screen} ref={screenRef} tabIndex={0}>
            <div className={styles.screenHeading}>
              <span>{modeHeading()}</span>
              <small>{halted ? "SCROLL HOLD" : "LIVE FOLLOW"}</small>
            </div>
            ...
            {mode === "who" ? <section className={styles.tableView}>
              <p className={styles.prompt}>WHO&gt; ENUMERATING REAL GREAT HALL MEMBERS...</p>
              <div className={styles.asciiTable} role="table" aria-label="Community members">
                <div className={styles.tableHeader} role="row"><span>CALLER</span><span>STATE</span><span>LAST SIGNAL</span></div>
                {members.length ? members.map((member) => <div key={member.pubkey} className={styles.tableRow} role="row"><strong>{member.displayName}</strong><span data-presence={member.presence}>{member.presence || "offline"}</span><span>{displayTime(member.updatedAt)}</span></div>) : <p className={styles.dim}>NO MEMBER RECORDS RETURNED.</p>}
              </div>
            </section> : null}

            {mode === "agents" ? <section className={styles.tableView}>
              <p className={styles.prompt}>AGENT&gt; READING GUILDHALL DIRECTORY...</p>
              <div className={styles.agentList}>
                {BUZZ_GUILDHALL_ACTORS.map((actor) => {
                  const room = roomById.get(actor.primaryChannel);
                  return <article key={actor.id}><header><strong>{actor.displayName}</strong><span>{actor.title}</span></header><p>{actor.summary}</p><footer>{actor.runtime.toUpperCase()} · {room?.label || actor.primaryChannel} · {actor.buzzPresence}</footer></article>;
                })}
              </div>
            </section> : null}

            {mode === "boards" ? <section className={styles.tableView}>
              <p className={styles.prompt}>BOARD&gt; MAPPING AVAILABLE CIRCLES...</p>
              <div className={styles.boardList}>
                {BUZZ_GUILDHALL_CHANNELS.map((channel) => {
                  const ready = roomById.has(channel.id);
                  return <article key={channel.id} data-ready={ready ? "true" : "false"}><header><strong>{channel.label}</strong><span>{ready ? "ONLINE" : "UNAVAILABLE"}</span></header><p>{channel.description}</p></article>;
                })}
                {storyRooms.map((room) => <article key={room.roomId} data-ready="true"><header><strong>{room.channel.name}</strong><span>STORY ROOM</span></header><p>{room.channel.description || "Private story conversation channel."}</p></article>)}
              </div>
            </section> : null}

            {mode === "talk" ? <section className={styles.talkView}>
              <p className={styles.prompt}>TALK&gt; CHOOSE A REAL PLOTPICKLE IDENTITY.</p>
              <div className={styles.talkLayout}>
                <div className={styles.targetList} aria-label="Talk targets">
                  <h3>AGENTS / PEOPLE</h3>
                  {talkTargets.map((target) => <button key={target.key} type="button" data-selected={selectedTargetKey === target.key ? "true" : "false"} onClick={() => setSelectedTargetKey(target.key)}><strong>{target.label}</strong><small>{target.detail}</small></button>)}
                </div>
                <div className={styles.directLine}>
                  {selectedTarget ? <>
                    <header><div><span>DIRECT LINE</span><strong>{selectedTarget.label}</strong></div><small>{selectedTarget.routeLabel}</small></header>
                    <div className={styles.directMessages} aria-live="polite">
                      {talkMessages.length ? talkMessages.map((message) => <article key={message.id}><header><strong>{message.author || "UNKNOWN"}</strong><time>{displayTime(message.createdAt)}</time></header><p>{message.content}</p></article>) : <p className={styles.dim}>{talkBusy ? "OPENING LINK..." : "NO RECENT PACKETS ON THIS ROUTE."}</p>}
                    </div>
                    <label><span>MESSAGE TO {selectedTarget.label.toUpperCase()}</span><textarea value={talkDraft} onChange={(event) => setTalkDraft(event.target.value)} rows={4} maxLength={8000} placeholder="Type message..." /></label>
                    <button type="button" disabled={!connected || !talkDraft.trim() || talkBusy} onClick={() => void sendTalkMessage()}>{talkBusy ? "TRANSMITTING..." : "[ENTER] SEND SIGNED PACKET"}</button>
                    <p className={styles.routeNotice}>{talkNotice || selectedTarget.routeLabel}</p>
                  </> : <p className={styles.dim}>SELECT AN IDENTITY FROM THE LEFT COLUMN TO OPEN A ROUTE.</p>}
                </div>
              </div>
            </section> : null}

            {mode === "reviews" ? <section className={styles.tableView}>
              <p className={styles.prompt}>REVIEW&gt; READING WRITER-OWNED DECISION QUEUE...</p>
              <div className={styles.reviewList}>
                {reviews.length ? reviews.map((review, index) => <article key={review.id}><span>#{String(index + 1).padStart(3, "0")}</span><div><strong>{review.title}</strong><small>{review.roomId || "COMMUNITY PROPOSAL"} · {review.status.toUpperCase()}</small></div></article>) : <p className={styles.dim}>QUEUE EMPTY. NO COMMUNITY CHANGE IS WAITING TO ALTER CANON.</p>}
              </div>
            </section> : null}

            {mode === "help" ? <section className={styles.helpView}>
              <p className={styles.prompt}>HELP&gt; KEYBOARD COMMAND TABLE.</p>
              {COMMANDS.map((command) => <p key={command.key}><kbd>[{command.key}]</kbd><strong>{command.label}</strong><span>{command.detail}</span></p>)}
              <p className={styles.warning}>KEYS ARE DISABLED WHILE YOU TYPE IN INPUTS, SELECTS OR TEXTAREAS. THIS TERMINAL NEVER EXECUTES OS/SHELL COMMANDS.</p>
              <button type="button" onClick={() => setHalted((current) => !current)}>{halted ? "[H] RESUME AUTO-FOLLOW" : "[H] HOLD AUTO-FOLLOW"}</button>
            </section> : null}
