...
function parameterScore(role, parameterB, fit) {
  ...
  if (role === "repair") {
    ...
    if (parameterB > 14 && parameterB <= 32) return fit.id === "gpu" ? 58 : 10;
...
export function scoreModelForRole(role, modelInput, hardware = {}) {
  ...
  if (cap.coding) {
    if (role === "repair") score += 34;
    else if (role === "fast") score -= 8;
    else score += 6;
    reasons.push(role === "fast" ? "coding specialization deprioritized for Fast" : "coding tuned");
  }
  if (cap.thinking) { score += role === "deep" ? 28 : role === "quality" ? 12 : role === "repair" ? 18 : 5; reasons.push("thinking/reasoning"); }
