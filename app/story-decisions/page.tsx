  ...
  return <main className={styles.page}>
    ...
    <div className={styles.layout}>
      ...
      <section className={styles.detail} aria-live="polite">{selected ? <>
        ...
        <section><h3>Your response</h3><div className={styles.actions}><button disabled={busy || !selected.proposedChange || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("accept-proposal")}>Accept recommendation</button><button disabled={busy || !selected.proposedChange || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("reject-proposal")}>Reject recommendation</button><button disabled={busy || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("keep-current")}>Keep current story</button><button disabled={busy || selected.status === "stale" || selected.baseRevision !== project.revision} onClick={() => void respond("request-alternatives")}>Ask for alternatives</button><button disabled={busy} onClick={() => void respond("defer")}>Defer</button><Link href="/?workspace=build">Open in story</Link></div>
