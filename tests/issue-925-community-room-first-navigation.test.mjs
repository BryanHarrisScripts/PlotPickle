...
test("#925 Community navigation progressively discloses real room-first destinations", async () => {
  ...
  for (const contract of [
    ...
    'aria-expanded={expanded.storyRooms}',
    'aria-expanded={expanded.guildhall}',
    '>Great Hall</button>',
    "storyRooms.map",
    "data-community-story-room-nav",
    "guildhallReadyRooms.map",
    "data-community-guildhall-room-nav",
    'aria-selected={section === "great-hall"}',
    'aria-selected={section === "story-rooms"}',
  ]) {
    assert.ok(rail.includes(contract), `Missing room-first Community navigation contract: ${contract}`);
  }
...
test("#925 Guildhall main pane shows one selected room rather than a flat room-card wall", async () => {
  ...
  for (const contract of [
    "selectedGuildhallRoomId",
    "selectedGuildhallDefinition",
    "onOpenGuildhallRoom",
    "data-guildhall-room-detail",
    "CommunityNavigationRail",
  ]) {
    assert.ok(workspace.includes(contract), `Missing selected Guildhall room contract: ${contract}`);
  }
...
test("#925 Community rail is sticky on desktop and collapses into normal flow on narrow screens", async () => {
  const css = await source("app/community-workspace.module.css");

  for (const contract of [
    ".communityRail",
    "position: sticky",
    ".communityRailGroup",
    ".communityRailChildren",
    "grid-template-columns: minmax(220px, 270px) minmax(0, 1fr)",
    "@media (max-width: 900px)",
    "position: static",
  ]) {
    assert.ok(css.includes(contract), `Missing responsive Community rail contract: ${contract}`);
  }
});
