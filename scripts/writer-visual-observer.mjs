...
export async function observeRenderedUi(client, resultText) {
  const result = await client.call("browser_evaluate", { function: `() => {
    ...
    for (const node of candidates) {
      ...
      lightSurfaces.push({
        ...
        label: (node.getAttribute('aria-label') || node.textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 90),
        background: style.backgroundColor,
        rect: rect(node)
      });
      if (lightSurfaces.length >= 8) break;
    }
    const controls = [...document.querySelectorAll('button, a, input, textarea, select, summary, [role="button"], [role="tab"]')].filter(visible);
    const clippedControls = controls.map((node) => ({
      label: (node.getAttribute('aria-label') || node.textContent || node.getAttribute('placeholder') || '').replace(/\\s+/g, ' ').trim().slice(0, 90),
      rect: rect(node)
    ...
    for (let i = 0; i < controls.length; i += 1) {
      ...
      for (let j = i + 1; j < controls.length; j += 1) {
        ...
        if (width > 4 && height > 4) {
          overlaps.push({
            first: (controls[i].getAttribute('aria-label') || controls[i].textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 70),
            second: (controls[j].getAttribute('aria-label') || controls[j].textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 70),
    ...
    const majorRegions = [...document.querySelectorAll('main > section, main > article, main > aside, main > nav, main > details')]
      ...
      .map((node) => ({
        tag: node.tagName.toLowerCase(),
        label: (node.getAttribute('aria-label') || node.querySelector('h1,h2,h3')?.textContent || '').replace(/\\s+/g, ' ').trim().slice(0, 80),
        rect: rect(node)
      }));
    ...
    return {
      ...
      main: mainRect ? {
        rect: mainRect,
        centerOffset: Math.round((mainRect.x + mainRect.width / 2) - viewport.width / 2),
        leftGap: Math.round(leftGap),
        rightGap: Math.round(rightGap),
        gapImbalance: Math.round(Math.abs(leftGap - rightGap))
      } : null,
  ...
  if (marker < 0 || end <= marker) return { error: "Visual observer returned no rendered JSON snapshot." };
  try { return JSON.parse(text.slice(marker, end + 1)); }
  catch { return { error: "Visual observer returned malformed rendered JSON." }; }
}
...
export function reviewRenderedUi(label, facts) {
  ...
  if (facts.theme !== "dark") {
    findings.push(finding("need", "high", `${label}: the screen is not using PlotPickle's current dark visual theme.`, "Active PlotPickle surfaces should share the current matte-black visual system.", "The screen can feel like a different or older product."));
  }
  ...
  if (Number(facts.horizontalOverflow || 0) > 8) {
    findings.push(finding("friction", "high", `${label}: the rendered page overflows the viewport horizontally by ${facts.horizontalOverflow}px.`, "The main workflow should fit the available window without sideways page scrolling.", "Controls or content can be missed and the layout feels unfinished."));
  }
  ...
  if ((facts.overlaps || []).length) {
    ...
    findings.push(finding("bug", "high", `${label}: visible controls overlap (${sample.first || "control"} / ${sample.second || "control"}).`, "Interactive controls should have clear, non-overlapping hit areas.", "Navigation becomes ambiguous and visually broken."));
  }
  const main = facts.main;
  if (main && main.rect?.width < facts.viewport.width * 0.94 && main.gapImbalance > 48) {
    findings.push(finding("friction", "medium", `${label}: the primary content area is visibly off-balance by about ${main.gapImbalance}px between left and right margins.`, "A deliberately constrained layout should still feel visually balanced unless the asymmetry serves a clear workflow purpose.", "The screen can feel accidentally shifted rather than intentionally composed."));
  }
  return findings;
}
