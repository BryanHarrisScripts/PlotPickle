        ...
        <div className={`pp-skin-v1-bbs ${styles.panel}`} data-skin-reference-panel="standard">
          ...
          <div className={`pp-skin-v1-menu pp-skin-v1-dashboard-menu ${styles.menu}`} role="listbox" aria-label={`${openCourseContent.title} lessons`}>
            {openCourseContent.lessons.map((lesson, index) => {
              ...
              return <button ref={(node) => { lessonRefs.current[index] = node; }} key={lesson.id} type="button" role="option" aria-selected={selected} tabIndex={selected ? 0 : -1} autoFocus={index === 0} className={`pp-skin-v1-menu-item pp-skin-v1-dashboard-row pp-skin-v1-submenu-item ${styles.row}${selected ? " is-selected" : ""}`} data-skin-menu-row={lesson.id} data-skin-menu-shortcut={shortcut} data-skin-menu-connected="true" data-learn-lesson-completed={completed ? "true" : "false"} onClick={() => openCourseLesson(index)} onKeyDown={(event) => handleLessonKeyDown(event, index)}><span className="pp-skin-v1-dashboard-command-line">[{shortcut}] {lesson.title}</span><span className={`pp-skin-v1-dashboard-status-box${completed ? " is-active" : ""}`} aria-label={completed ? "Lesson complete" : "Lesson incomplete"} data-dashboard-status={completed ? "active" : "inactive"} data-skin-menu-indicator="connected" /></button>;
            })}
          </div>
          <p className={`pp-skin-v1-bbs-help ${styles.help}`} role="status">{notice}</p>
        </div>
        ...
        <div className={`pp-skin-v1-bbs ${styles.panel}`} data-skin-reference-panel="standard">
          ...
          <div className={`pp-skin-v1-menu pp-skin-v1-dashboard-menu ${styles.menu}`} role="listbox" aria-label={`${pathLabel} Craft Modules`} aria-describedby="learn-journey-course-status">
            {semester.courses.map((course, index) => {
              ...
              return <button ref={(node) => { courseRefs.current[index] = node; }} key={course.id} type="button" role="option" aria-selected={selected} tabIndex={selected ? 0 : -1} autoFocus={index === 0} className={`pp-skin-v1-menu-item pp-skin-v1-dashboard-row pp-skin-v1-submenu-item ${styles.row}${selected ? " is-selected" : ""}`} data-skin-menu-row={course.id} data-skin-menu-shortcut={shortcut} data-skin-menu-connected="true" data-learn-course-status="wired" data-learn-course-content="available" data-learn-course-progress={progress} onClick={() => void openCourse(index)} onKeyDown={(event) => handleCourseKeyDown(event, index)}><span className="pp-skin-v1-dashboard-command-line">{command} · {progress}</span><span className="pp-skin-v1-dashboard-status-box is-active" aria-label="Craft Module connected" data-dashboard-status="active" data-skin-menu-indicator="connected" /></button>;
            })}
          </div>
