...
function findingMarkdown(candidate, index) {
  ...
  const status = candidate.reviewQualification === "qualified"
    ...
    : area
      ? `Below qualification threshold by ${Number(candidate.qualificationGap || 0).toFixed(2)} points; included because it ranked in today's ${area.label} Top 3.`
  ...
  return [
    ...
    `- **Review history:** ${candidate.previouslyReviewed ? (area ? `seen previously; resurfaced because ${candidate.resurfacingReason}` : "seen in a previous Radar review") : "first Radar review"}`,
    `- **Score evidence:** ${sortedUnique(candidate?.scoreEvidence?.reasonCodes || []).join(", ") || "no reason codes"}`,
    ...(candidate.ossRulesSignal ? [`- **OSS Rules signal:** ${candidate.ossRulesSignal.pattern} — ${candidate.ossRulesSignal.disposition}; [pinned source](${candidate.ossRulesSignal.sourceUrl})`] : []),
    "",
  ].join("\n");
}
...
export function renderDailyReport({ reportDate, selection, contract, history = new Map() }) {
  ...
  return {
    body: [
      ...
      reviewNote,
      "",
      adaptive ? "Classifications are deterministic Radar guidance, not adoption decisions. Human review remains authoritative." : "Phase 3 classifications are deterministic Radar guidance, not adoption decisions. Human review remains authoritative.",
      "",
