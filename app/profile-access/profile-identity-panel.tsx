...
export default function ProfileIdentityPanel({
  ...
  async function saveLocalPresentation(next: Presentation) {
    return jsonRequest<{ profile: Presentation; localSaved: true }>("/api/auth/profile-presentation", {
      method: "POST",
      headers: { "X-PlotPickle-CSRF": csrfToken },
  ...
  async function savePresentation(event: FormEvent) {
    ...
    try {
      const result = await saveLocalPresentation(presentation);
      setPresentation(result.profile);
      await onProfileChanged();
      if (buzzStatus?.connection?.identityConfigured) {
        if (isPlotPickleGeneratedAvatarRef(result.profile.avatarUrl)) {
          setNotice("Profile saved locally. Your generated Lore Avatar stays private to this PlotPickle profile until BUZZ supports a publicly reachable image for it; the existing BUZZ avatar was not replaced.");
        } else {
          try {
  ...
  async function generateLoreAvatar() {
    ...
    const billingAcknowledged = window.confirm("Generate one Lore Avatar with your selected image route? A selected cloud provider may charge the API account you configured. PlotPickle does not supply credits or silently switch providers.");
    if (!billingAcknowledged) {
      setNotice("Lore Avatar generation was cancelled. Your current avatar was not changed.");
      return;
    }
    setBusy("avatar"); setNotice("");
    try {
      const prompt = buildHumanLoreAvatarPrompt(description);
      const response = await fetch("/api/local-ai/generate/image", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt,
          assetId: `human-lore-avatar-${profile.profileId}`,
          aspect: "portrait",
          quality: "high",
          requestCount: 1,
          billingAcknowledged,
      ...
      const generated = await response.json() as ImageResponse;
  ...
  async function finishBuzzSetup() {
    ...
    try {
      ...
      try {
        const published = isPlotPickleGeneratedAvatarRef(presentation.avatarUrl) ? null : await publishToBuzz(presentation);
        setNotice(published?.message || (isPlotPickleGeneratedAvatarRef(presentation.avatarUrl)
          ? `BUZZ identity connected to ${PLOTPICKLE_BUZZ_COMMUNITY.displayName}. Your generated Lore Avatar remains local and the BUZZ avatar was not replaced.`
          : `BUZZ identity connected to ${PLOTPICKLE_BUZZ_COMMUNITY.displayName} and your Profile was published.`));
      } catch (cause) {
