...
export function registerAutonomousGuestRoutingStatus(server: ViteDevServer) {
  server.middlewares.use((request, response, next) => {
    ...
    void (async () => {
      ...
      sendJson(response, 200, guestStatus(authority));
    })().catch((error) => {
      if (response.headersSent) return;
      sendJson(response, 500, { ok: false, message: error instanceof Error ? error.message : "Autonomous Guest AI status is unavailable." });
    });
  });
}
