...
export async function verifyWebMcpZip(zipPath) {
  const info = await stat(zipPath);
  if (!info.isFile() || info.size <= 22) {
    throw new Error(`WebMCP Full QA ZIP was not created correctly: ${zipPath}`);
  }
  ...
  try {
    ...
    if (bytesRead !== signature.length || signature.readUInt32LE(0) !== 0x04034b50) {
      throw new Error(`WebMCP Full QA ZIP is not readable as a ZIP archive: ${zipPath}`);
    }
  } finally {
    await handle.close();
  }

  return { zipPath, size: info.size, verified: true };
}
...
export async function finalizeWebMcpFullQaArtifacts({
  ...
  const zipVerification = await verifyWebMcpZip(zipPath);

  const latestZipPath = path.resolve(repoRoot, ".artifacts/webmcp-full-qa-latest.zip");
  await cp(zipPath, latestZipPath, { force: true });
  const latestZipVerification = await verifyWebMcpZip(latestZipPath);
